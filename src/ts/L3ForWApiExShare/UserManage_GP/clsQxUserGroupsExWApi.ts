...
import {
  QxUserGroups_GetObjLstByPagerAsync,
  QxUserGroups_SortFunByKey,
} from '@/ts/L3ForWApi/UserManage_GP/clsQxUserGroupsWApi';
 ...
 **/
export function QxUserGroupsEx_CopyToEx(
  objQxUserGroupsENS: clsQxUserGroupsEN,
): clsQxUserGroupsENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      qxUserGroupsEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function QxUserGroupsEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsQxUserGroupsENEx>> {
  ...
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrQxUserGroupsSel = arrQxUserGroupsSel.sort(
        QxUserGroupsEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function QxUserGroupsEx_FuncMapByFldName(
  strFldName: string,
  objQxUserGroupsEx: clsQxUserGroupsENEx,
) {
  ...
  let strMsg = '';
  //如果是本表中字段,不需要映射
  const arrFldName = clsQxUserGroupsEN._AttributeName;
