...
if (clsQxUserStateEN._AttributeName.indexOf(strOutFldName) == -1)
{
const strMsg = Format("输出字段名:[{0}]不正确,不在输出字段范围之内!({1})",
strOutFldName, clsQxUserStateEN._AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (IsNullOrEmpty(clsQxUserStateEN._WhereFormat) == false)
{
strWhereCond = clsQxUserStateEN._WhereFormat;
}
if (IsNullOrEmpty(clsQxUserStateEN._CacheAddiCondition) == false)
{
strWhereCond += Format(" and {0}", clsQxUserStateEN._CacheAddiCondition);
}
if (strKey == "")
{
console.error("关键字为空!不正确");
...
switch (clsQxUserStateEN._CacheModeId)
{
 ...
 * @returns 对象列表
*/
export  async function QxUserState_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsQxUserStateENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
...
await QxUserState_FuncMapByFldName(strFldName, newObj);
}
}
...
clsQxUserStateEN._AttributeName.indexOf(objSortInfo.SortFld) == -1 &&
(bolIsFuncMap == false || bolIsFuncMap == undefined)
)
{
for (const newObj of arrQxUserStateExObjLst) {
await QxUserState_FuncMapByFldName(objSortInfo.SortFld, newObj);
...
const intEnd = intStart + objPagerPara.pageSize;
if (objPagerPara.orderBy != null && objPagerPara.orderBy.length>0) {
arrQxUserStateSel = arrQxUserStateSel.sort(QxUserState_SortFunByExKey(objSortInfo.SortFld, objSortInfo.SortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrQxUserStateSel = arrQxUserStateSel.sort(objPagerPara.sortFun);
...
{
const strMsg = Format("(errid:Watl001294)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxUserState_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objQxUserStateENT;
}
...
*/
export  function QxUserState_FuncMapByFldName(strFldName: string, objQxUserStateEx: clsQxUserStateENEx)
{
const strThisFuncName = QxUserState_FuncMapByFldName.name;
console.log(objQxUserStateEx);
strFldName = strFldName.replace('|Ex', '');
let strMsg = "";
//如果是本表中字段,不需要映射
const arrFldName = clsQxUserStateEN._AttributeName;
 ...
 * @returns 返回两个对象比较的结果
*/
export  function QxUserState_SortFunByExKey(strKey:string, AscOrDesc: string)
{
