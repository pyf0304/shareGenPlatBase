...
export async function CacheMode_func(
  ...
  if (clsCacheModeEN._AttributeName.indexOf(strOutFldName) == -1) {
    const strMsg = Format(
      ...
      strOutFldName,
      clsCacheModeEN._AttributeName.join(','),
    );
    console.error(strMsg);
    throw new Error(strMsg);
  }
...
export async function CacheMode_GetSubObjLstCache(objCacheModeCond: clsCacheModeEN) {
  ...
  if (objCacheModeCond.sfFldComparisonOp == null || objCacheModeCond.sfFldComparisonOp == '')
    return arrCacheModeSel;
  const dicFldComparisonOp: { [index: string]: string } = JSON.parse(
    objCacheModeCond.sfFldComparisonOp,
  );
...
export function CacheMode_CheckPropertyNew(pobjCacheModeEN: clsCacheModeEN) {
  ...
  if (IsNullOrEmpty(pobjCacheModeEN.memo) == false && GetStrLen(pobjCacheModeEN.memo) > 1000) {
    throw new Error(
      `(errid:Watl000413)字段[备注(memo)]的长度不能超过1000(In 缓存方式(CacheMode))!值:${pobjCacheModeEN.memo}(clsCacheModeBL:CheckPropertyNew)`,
    );
  }
  //检查字段的数据类型是否正确
  if (
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000414)字段[备注(memo)]的值:[${pobjCacheModeEN.memo}], 非法,应该为字符型(In 缓存方式(CacheMode))!(clsCacheModeBL:CheckPropertyNew0)`,
    );
  }
...
export function CacheMode_CheckProperty4Update(pobjCacheModeEN: clsCacheModeEN) {
  ...
  if (IsNullOrEmpty(pobjCacheModeEN.memo) == false && GetStrLen(pobjCacheModeEN.memo) > 1000) {
    throw new Error(
      `(errid:Watl000416)字段[备注(memo)]的长度不能超过1000(In 缓存方式(CacheMode))!值:${pobjCacheModeEN.memo}(clsCacheModeBL:CheckProperty4Update)`,
    );
  }
  //检查字段的数据类型是否正确
  if (
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000417)字段[备注(memo)]的值:[${pobjCacheModeEN.memo}], 非法,应该为字符型(In 缓存方式(CacheMode))!(clsCacheModeBL:CheckProperty4Update)`,
    );
  }
  //检查主键是否为Null或者空!
  //检查外键, 作为外键应该和主键的字段长度是一样的, 若不一样,即非法!
 ...
 */
export function CacheMode_CopyObjTo(
  objCacheModeENS: clsCacheModeEN,
  objCacheModeENT: clsCacheModeEN,
): void {
