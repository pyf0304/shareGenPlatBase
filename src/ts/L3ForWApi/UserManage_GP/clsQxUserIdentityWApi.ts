...
import { qxUserIdentityCache,isFuncMapCache } from "@/viewsShare/UserManage_GP/QxUserIdentityVueShare";
 ...
 * @returns 对象列表
*/
export  async function QxUserIdentity_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsQxUserIdentityENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
...
await QxUserIdentity_FuncMapByFldName(strFldName, newObj);
}
}
...
await QxUserIdentity_FuncMapByFldName(objSortInfo.SortFld, newObj);
...
const intEnd = intStart + objPagerPara.pageSize;
if (objPagerPara.orderBy != null && objPagerPara.orderBy.length>0) {
arrQxUserIdentitySel = arrQxUserIdentitySel.sort(QxUserIdentity_SortFunByExKey(objSortInfo.SortFld, objSortInfo.SortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrQxUserIdentitySel = arrQxUserIdentitySel.sort(objPagerPara.sortFun);
 ...
 **/
export  function QxUserIdentity_CopyToEx(objQxUserIdentityENS:clsQxUserIdentityEN ): clsQxUserIdentityENEx
{
...
{
const strMsg = Format("(errid:Watl001294)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxUserIdentity_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objQxUserIdentityENT;
}
...
*/
export  function QxUserIdentity_FuncMapByFldName(strFldName: string, objQxUserIdentityEx: clsQxUserIdentityENEx)
{
const strThisFuncName = QxUserIdentity_FuncMapByFldName.name;
 ...
 * @returns 返回两个对象比较的结果
*/
export  function QxUserIdentity_SortFunByExKey(strKey:string, AscOrDesc: string)
{
