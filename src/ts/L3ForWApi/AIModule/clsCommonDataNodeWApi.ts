 ...
 * @returns 对象
 **/
export  async function CommonDataNode_GetObjBymIdAsync(lngmId: number): Promise<clsCommonDataNodeEN|null>  
{
...
if (lngmId == 0)
{
  const strMsg = Format("参数:[lngmId]不能为空!(In clsCommonDataNodeWApi.GetObjBymIdlocalStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
{
const strMsg = Format("错误:[{0}]. \n根据关键字:[{1}]获取相应的对象不成功!(in {2}.{3})", e, lngmId, commonDataNode_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
return;
}
...
try
{
const arrCommonDataNodeSel = arrCommonDataNodeObjLstCache.filter(x => 
 x.mId == lngmId );
...
{
const arrCommonDataNodeObjLstCache = await CommonDataNode_GetObjLstCache();
const obj = arrCommonDataNodeObjLstCache.find(x => x.dateTimeSim == objCommonDataNode.dateTimeSim);
if (obj != null)
{
...
const strMsg = Format("错误:[{0}]. \n在列表中修改对象不成功!(in {1}.{2})", e, commonDataNode_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
 ...
 * @returns 返回一个输出字段值
*/
export  async function CommonDataNode_func(strInFldName:string , strOutFldName:string , strInValue:string )
{
 ...
 * @returns 返回一个关键字值列表
*/
export  async function CommonDataNode_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<number>> 
{
...
arrCommonDataNodeSel = arrCommonDataNodeSel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
break;
case enumComparisonOp.Like_03:
arrCommonDataNodeSel = arrCommonDataNodeSel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
break;
case enumComparisonOp.In_04:
arrValues = strInValue.split(',');
arrCommonDataNodeSel = arrCommonDataNodeSel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
break;
}
break;
case "boolean":
...
{
arrCommonDataNodeSel = arrCommonDataNodeSel.filter(x => x.GetFldValue(strInFldName) == strInValue);
}
break;
case "number":
 ...
 **/
export  async function CommonDataNode_GetFldValueAsync(strFldName: string, strWhereCond: string): Promise<Array<string>>  
{
 ...
 * @returns 第一条记录对象
 **/
export  async function CommonDataNode_GetFirstObjAsync(strWhereCond: string): Promise<clsCommonDataNodeEN|null>  
{
...
const arrCommonDataNodeObjLstT = CommonDataNode_GetObjLstByJSONObjLst(arrCommonDataNodeExObjLstCache);
...
const strInfo = Format("[ClientCache]Key:[{0}]的缓存已经建立,对象列表数：{1}!", strKey, arrCommonDataNodeExObjLst.length);
console.log(strInfo);
return arrCommonDataNodeExObjLst;
}
catch (e)
{
const strMsg = Format("从本地缓存中获取所有对象列表出错. \n服务器错误：{0}.(in {1}.{2})", e, commonDataNode_ConstructorName, strThisFuncName);
console.error(strMsg);
throw (strMsg);
}
}
...
const strInfo = Format("[localStorage]Key:[{0}]的缓存已经建立,对象列表数：{1}!", strKey, arrCommonDataNodeExObjLst.length);
console.log(strInfo);
return arrCommonDataNodeExObjLst;
}
catch (e)
 ...
 * @returns 获取的相应对象列表
 **/
export  async function CommonDataNode_GetObjLstAsync(strWhereCond: string): Promise<Array<clsCommonDataNodeEN>>  
{
...
const strInfo = Format("[sessionStorage]Key:[{0}]的缓存已经建立,对象列表数：{1}!", strKey, arrCommonDataNodeExObjLst.length);
console.log(strInfo);
return arrCommonDataNodeExObjLst;
}
catch (e)
 ...
 * @returns 对象列表子集
*/
export  async function CommonDataNode_GetSubObjLstCache(objCommonDataNodeCond: ConditionCollection ) {
const strThisFuncName = "GetSubObjLstCache";
const arrCommonDataNodeObjLstCache = await CommonDataNode_GetObjLstCache();
let arrCommonDataNodeSel = arrCommonDataNodeObjLstCache;
if (objCommonDataNodeCond.GetConditions().length == 0) return arrCommonDataNodeSel;
try {
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]缓存对象列表中获取子集对象不成功!(in {2}.{3})", e, JSON.stringify( objCommonDataNodeCond), commonDataNode_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 对象列表
 **/
export  async function CommonDataNode_GetObjLstBymIdLstAsync(arrmId: Array<string>): Promise<Array<clsCommonDataNodeEN>>  
{
...
{
const arrCommonDataNodeObjLstCache = await CommonDataNode_GetObjLstCache();
const arrCommonDataNodeSel = arrCommonDataNodeObjLstCache.filter(x => arrmIdLst.indexOf(x.mId)>-1);
return arrCommonDataNodeSel;
}
catch (e)
{
const strMsg = Format("错误:[{0}]. \n根据关键字:[{1}]获取对象列表不成功!(in {2}.{3})", e, arrmIdLst.join(","), commonDataNode_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
}
 ...
 * @returns 获取的相应对象列表
 **/
export  async function CommonDataNode_GetTopObjLstAsync(objTopPara: stuTopPara): Promise<Array<clsCommonDataNodeEN>>  
{
 ...
 * @returns 获取的相应记录对象列表
 **/
export  async function CommonDataNode_GetObjLstByRangeAsync(objRangePara: stuRangePara): Promise<Array<clsCommonDataNodeEN>>  
{
...
let strUrl = GetWebApiUrl_GP(commonDataNode_Controller, strAction);
 ...
 * @returns 实际删除记录的个数
 **/
export  async function CommonDataNode_DelCommonDataNodesAsync(arrmId: Array<string>): Promise<number> 
{
 ...
 * @returns 实际删除记录的个数
 **/
export  async function CommonDataNode_DelCommonDataNodesByCondAsync(strWhereCond: string): Promise<number> 
{
 ...
 * @returns 获取相应的记录的对象
 **/
export  async function CommonDataNode_AddNewRecordAsync(objCommonDataNodeEN: clsCommonDataNodeEN): Promise<boolean> 
{
...
try
{
CommonDataNode_CheckPropertyNew(objCommonDataNodeEN);
}
catch(e)
{
const strMsg = `检查数据不成功,${e}.(in ${ commonDataNode_ConstructorName }.${strThisFuncName})`;
console.error(strMsg);
alert(strMsg);
...
{
//检查唯一性条件
const bolIsExistCond = await CommonDataNode_CheckUniCond4Add(objCommonDataNodeEN);
if (bolIsExistCond == false)
{
return { keyword: '', success: false };
}
let returnBool = false;
returnBool = await CommonDataNode_AddNewRecordAsync(objCommonDataNodeEN);
if (returnBool == true)
{
CommonDataNode_ReFreshCache();
}
else
{
const strInfo = `添加[公共数据结点(CommonDataNode)]记录不成功!`;
//显示信息框
throw(strInfo);
}
return { keyword: objCommonDataNodeEN.mId.toString(), success: returnBool };//一定要有一个返回值,否则会出错!
}
catch(e)
{
const strMsg = `添加记录不成功,${e}.(in ${ commonDataNode_ConstructorName }.${ strThisFuncName })`;
console.error(strMsg);
throw(strMsg);
}
}
 ...
 **/
export  async function CommonDataNode_CheckUniCond4Add(objCommonDataNodeEN: clsCommonDataNodeEN): Promise<boolean>{
const strUniquenessCondition = CommonDataNode_GetUniCondStr(objCommonDataNodeEN);
const bolIsExistCondition = await CommonDataNode_IsExistRecordAsync(strUniquenessCondition);
 ...
 **/
export  async function CommonDataNode_CheckUniCond4Update(objCommonDataNodeEN: clsCommonDataNodeEN): Promise<boolean>{
const strUniquenessCondition = CommonDataNode_GetUniCondStr4Update(objCommonDataNodeEN);
const bolIsExistCondition = await CommonDataNode_IsExistRecordAsync(strUniquenessCondition);
if (bolIsExistCondition == true)
{
const strMsg = Format("不能满足唯一性条件。满足条件：{0}的记录已经存在!", strUniquenessCondition);
 ...
 **/
export  async function CommonDataNode_UpdateObjSave(objCommonDataNodeEN: clsCommonDataNodeEN): Promise<boolean>{
const strThisFuncName = 'UpdateObjSave';
objCommonDataNodeEN.sfUpdFldSetStr = objCommonDataNodeEN.updFldString;//设置哪些字段被修改(脏字段)
if (objCommonDataNodeEN.mId == 0 || objCommonDataNodeEN.mId == undefined){
console.error("关键字不能为空!");
throw "关键字不能为空!";
}
try
{
CommonDataNode_CheckProperty4Update(objCommonDataNodeEN);
}
catch(e)
{
const strMsg = `检查数据不成功,${e}.(in ${ commonDataNode_ConstructorName }.${strThisFuncName})`;
...
{
//检查唯一性条件
const bolIsExistCond = await CommonDataNode_CheckUniCond4Update(objCommonDataNodeEN);
if (bolIsExistCond == false)
{
return false;
}
const returnBool = await CommonDataNode_UpdateRecordAsync(objCommonDataNodeEN);
if (returnBool == true)
{
CommonDataNode_ReFreshCache();
}
...
{
const strMsg = `修改记录不成功,${e}.(in ${ commonDataNode_ConstructorName }.${ strThisFuncName })`;
console.error(strMsg);
throw(strMsg);
}
}
 ...
 * @returns 返回新添加记录的关键字
 **/
export  async function CommonDataNode_AddNewRecordWithReturnKeyAsync(objCommonDataNodeEN: clsCommonDataNodeEN): Promise<string> 
{
 ...
 * @returns 获取修改是否成功？
 **/
export  async function CommonDataNode_UpdateRecordAsync(objCommonDataNodeEN: clsCommonDataNodeEN): Promise<boolean>  
{
const strThisFuncName = "UpdateRecordAsync";
const strAction = "UpdateRecord";
 if (objCommonDataNodeEN.sfUpdFldSetStr === undefined || objCommonDataNodeEN.sfUpdFldSetStr === null || objCommonDataNodeEN.sfUpdFldSetStr === "")
{
const strMsg = Format("对象(关键字: {0})的【修改字段集】为空,不能修改!", objCommonDataNodeEN.mId);
 throw strMsg;
 }
const strUrl = GetWebApiUrl_GP(commonDataNode_Controller, strAction);
 ...
 * @returns 获取修改是否成功？
 **/
export  async function CommonDataNode_EditRecordExAsync(objCommonDataNodeEN: clsCommonDataNodeEN): Promise<boolean>  
{
 ...
 * @returns 返回的第一条记录的关键字值
 **/
export  async function CommonDataNode_UpdateWithConditionAsync(objCommonDataNodeEN: clsCommonDataNodeEN, strWhereCond: string ): Promise<boolean> 
{
...
let arrCommonDataNodeSel = arrCommonDataNodeObjLstCache;
if (objCommonDataNodeCond.GetConditions().length == 0) return arrCommonDataNodeSel.length>0?true:false;
try {
for (const objCondition of objCommonDataNodeCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
...
const strMsg = Format("根据条件:[{0}]判断是否存在不成功!(in {1}.{2})", JSON.stringify( objCommonDataNodeCond), commonDataNode_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
return false;
...
{
const strMsg = Format("根据关键字:[{0}]判断是否存在不成功!(in {1}.{2})", lngmId, commonDataNode_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
return false;
 ...
 * @returns 对象列表记录数
*/
export  async function CommonDataNode_GetRecCountByCondCache(objCommonDataNodeCond: ConditionCollection ) {
const strThisFuncName = "GetRecCountByCondCache";
const arrCommonDataNodeObjLstCache = await CommonDataNode_GetObjLstCache();
if (arrCommonDataNodeObjLstCache == null) return 0;
let arrCommonDataNodeSel = arrCommonDataNodeObjLstCache;
if (objCommonDataNodeCond.GetConditions().length == 0) return arrCommonDataNodeSel.length;
try {
for (const objCondition of objCommonDataNodeCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]从缓存对象列表中获取记录数不成功!(in {2}.{3})", e, JSON.stringify( objCommonDataNodeCond), commonDataNode_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
return 0;
...
clsCommonDataNodeEN._RefreshTimeLst.push(clsDateTime.getTodayDateTimeStr(0));
}
...
**/
export  function CommonDataNode_GetLastRefreshTime():string
{
if (clsCommonDataNodeEN._RefreshTimeLst.length == 0) return "";
return clsCommonDataNodeEN._RefreshTimeLst[clsCommonDataNodeEN._RefreshTimeLst.length - 1];
}
...
if (IsNullOrEmpty(pobjCommonDataNodeEN.dateTimeSim) == false && GetStrLen(pobjCommonDataNodeEN.dateTimeSim) > 20)
{
 throw new Error(`(errid:Watl000413)字段[简化日期时间(dateTimeSim)]的长度不能超过20(In 公共数据结点(CommonDataNode))!值:${pobjCommonDataNodeEN.dateTimeSim}(clsCommonDataNodeBL:CheckPropertyNew)`);
}
//检查字段的数据类型是否正确
if (null != pobjCommonDataNodeEN.mId && undefined !== pobjCommonDataNodeEN.mId && tzDataType.isNumber(pobjCommonDataNodeEN.mId) === false)
{
 throw new Error(`(errid:Watl000414)字段[流水号(mId)]的值:[${pobjCommonDataNodeEN.mId}], 非法,应该为数值型(In 公共数据结点(CommonDataNode))!(clsCommonDataNodeBL:CheckPropertyNew0)`);
}
if (IsNullOrEmpty(pobjCommonDataNodeEN.dateTimeSim) == false && undefined !== pobjCommonDataNodeEN.dateTimeSim && tzDataType.isString(pobjCommonDataNodeEN.dateTimeSim) === false)
{
 throw new Error(`(errid:Watl000414)字段[简化日期时间(dateTimeSim)]的值:[${pobjCommonDataNodeEN.dateTimeSim}], 非法,应该为字符型(In 公共数据结点(CommonDataNode))!(clsCommonDataNodeBL:CheckPropertyNew0)`);
}
...
if (IsNullOrEmpty(pobjCommonDataNodeEN.dateTimeSim) == false && GetStrLen(pobjCommonDataNodeEN.dateTimeSim) > 20)
{
 throw new Error(`(errid:Watl000416)字段[简化日期时间(dateTimeSim)]的长度不能超过20(In 公共数据结点(CommonDataNode))!值:${pobjCommonDataNodeEN.dateTimeSim}(clsCommonDataNodeBL:CheckProperty4Update)`);
}
//检查字段的数据类型是否正确
if (null != pobjCommonDataNodeEN.mId && undefined !== pobjCommonDataNodeEN.mId && tzDataType.isNumber(pobjCommonDataNodeEN.mId) === false)
{
 throw new Error(`(errid:Watl000417)字段[流水号(mId)]的值:[${pobjCommonDataNodeEN.mId}], 非法,应该为数值型(In 公共数据结点(CommonDataNode))!(clsCommonDataNodeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjCommonDataNodeEN.dateTimeSim) == false && undefined !== pobjCommonDataNodeEN.dateTimeSim && tzDataType.isString(pobjCommonDataNodeEN.dateTimeSim) === false)
{
 throw new Error(`(errid:Watl000417)字段[简化日期时间(dateTimeSim)]的值:[${pobjCommonDataNodeEN.dateTimeSim}], 非法,应该为字符型(In 公共数据结点(CommonDataNode))!(clsCommonDataNodeBL:CheckProperty4Update)`);
}
//检查主键是否为Null或者空!
if (null === pobjCommonDataNodeEN.mId 
 || pobjCommonDataNodeEN.mId != null && pobjCommonDataNodeEN.mId.toString()  ===  "")
{
 throw new Error(`(errid:Watl000064)字段[流水号]不能为空(In 公共数据结点)!(clsCommonDataNodeBL:CheckProperty4Update)`);
}
//检查外键, 作为外键应该和主键的字段长度是一样的, 若不一样,即非法!
 ...
 * @returns 返回一个生成的对象列表
*/
export  function CommonDataNode_GetObjLstByJSONObjLst (arrCommonDataNodeObjLstS: Array<clsCommonDataNodeEN>): Array<clsCommonDataNodeEN>
{
...
*/
export  function CommonDataNode_GetCombineCondition(objCommonDataNodeCond: clsCommonDataNodeEN ):string
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
let strWhereCond = " 1 = 1 ";
//如果该条件控件的内容不为空,就组成一个条件并添加到总条件串中。
if (Object.prototype.hasOwnProperty.call(objCommonDataNodeCond.dicFldComparisonOp, clsCommonDataNodeEN.con_mId) == true)
{
const strComparisonOpmId:string = objCommonDataNodeCond.dicFldComparisonOp[clsCommonDataNodeEN.con_mId];
strWhereCond += Format(" And {0} {2} {1}", clsCommonDataNodeEN.con_mId, objCommonDataNodeCond.mId, strComparisonOpmId);
}
if (Object.prototype.hasOwnProperty.call(objCommonDataNodeCond.dicFldComparisonOp, clsCommonDataNodeEN.con_DateTimeSim) == true)
{
const strComparisonOpDateTimeSim:string = objCommonDataNodeCond.dicFldComparisonOp[clsCommonDataNodeEN.con_DateTimeSim];
strWhereCond += Format(" And {0} {2} '{1}'", clsCommonDataNodeEN.con_DateTimeSim, objCommonDataNodeCond.dateTimeSim, strComparisonOpDateTimeSim);
}
 return strWhereCond;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
export  function CommonDataNode_GetUniCondStr4Update(objCommonDataNodeEN: clsCommonDataNodeEN ):string
{
let strWhereCond = " 1 = 1 ";
...
*/
export  function CommonDataNode_GetObjFromJsonObj(objCommonDataNodeENS: clsCommonDataNodeEN): clsCommonDataNodeEN 
{
