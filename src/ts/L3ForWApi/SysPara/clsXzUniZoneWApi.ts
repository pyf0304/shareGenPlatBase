...
export async function XzUniZone_func(
  ...
  if (clsXzUniZoneEN._AttributeName.indexOf(strOutFldName) == -1) {
    const strMsg = Format(
      '输出字段名:[{0}]不正确,不在输出字段范围之内!({1})',
      strOutFldName,
      clsXzUniZoneEN._AttributeName.join(','),
    );
    console.error(strMsg);
    throw new Error(strMsg);
  }
 ...
 **/
export async function XzUniZone_GetFldValueAsync(
  strFldName: string,
  strWhereCond: string,
): Promise<Array<string>> {
 ...
 **/
export async function XzUniZone_AddNewObjSave(
  objXzUniZoneEN: clsXzUniZoneEN,
 ...
 * @returns 获取修改是否成功？
 **/
export async function XzUniZone_EditRecordExAsync(
  objXzUniZoneEN: clsXzUniZoneEN,
): Promise<boolean> {
...
export function XzUniZone_CheckPropertyNew(pobjXzUniZoneEN: clsXzUniZoneEN) {
  ...
  if (
    IsNullOrEmpty(pobjXzUniZoneEN.id_School) == false &&
    GetStrLen(pobjXzUniZoneEN.id_School) > 4
  ) {
    throw new Error(
      `(errid:Watl000413)字段[学校流水号(idSchool)]的长度不能超过4(In 校区(XzUniZone))!值:${pobjXzUniZoneEN.id_School}(clsXzUniZoneBL:CheckPropertyNew)`,
    );
  }
  ...
  if (
    IsNullOrEmpty(pobjXzUniZoneEN.id_School) == false &&
    undefined !== pobjXzUniZoneEN.id_School &&
    tzDataType.isString(pobjXzUniZoneEN.id_School) === false
  ) {
    throw new Error(
      `(errid:Watl000414)字段[学校流水号(idSchool)]的值:[${pobjXzUniZoneEN.id_School}], 非法,应该为字符型(In 校区(XzUniZone))!(clsXzUniZoneBL:CheckPropertyNew0)`,
    );
  }
  if (
...
export function XzUniZone_CheckProperty4Update(pobjXzUniZoneEN: clsXzUniZoneEN) {
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000416)字段[学校流水号(idSchool)]的长度不能超过4(In 校区(XzUniZone))!值:${pobjXzUniZoneEN.id_School}(clsXzUniZoneBL:CheckProperty4Update)`,
    );
  }
  if (IsNullOrEmpty(pobjXzUniZoneEN.idUni) == false && GetStrLen(pobjXzUniZoneEN.idUni) > 4) {
    throw new Error(
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000417)字段[学校流水号(idSchool)]的值:[${pobjXzUniZoneEN.id_School}], 非法,应该为字符型(In 校区(XzUniZone))!(clsXzUniZoneBL:CheckProperty4Update)`,
    );
  }
  if (
    IsNullOrEmpty(pobjXzUniZoneEN.idUni) == false &&
