...
import {
  CombineXzSchoolConditionObj4ExportExcel,
  CombineXzSchoolConditionObj,
  XzSchool_DeleteKeyIdCache,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
...
import {
  XzSchool_GetRecCountByCondCache,
  XzSchool_GetSubObjLstCache,
  XzSchool_DelRecordAsync,
  XzSchool_ReFreshCache,
  XzSchool_GetObjByIdSchoolAsync,
  XzSchool_GetObjLstByIdSchoolLstAsync,
  XzSchool_AddNewRecordAsync,
  XzSchool_UpdateRecordAsync,
  XzSchool_DelXzSchoolsAsync,
...
import {
  XzSchoolEx_FuncMapByFldName,
  XzSchoolEx_GetObjExLstByPagerCache,
} from '@/ts/L3ForWApiExShare/SystemSet/clsXzSchoolExWApi';
...
export abstract class XzSchoolCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortXzSchoolBy == '')
        viewVarSet.sortXzSchoolBy = `${clsXzSchoolEN.con_IdSchool} Asc`;
   ...
   **/
  public CombineData(
    arrXzSchoolObjLst: Array<clsXzSchoolEN>,
    ...
    const strFileName = Format('学校({0})导出.xlsx', clsXzSchoolEN._CurrTabName);
  ...
  public async ExportExcel_XzSchoolCache(): Promise<ExportExcelData> {
    ...
    if (viewVarSet.sortXzSchoolBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortXzSchoolBy)为空,请检查!(In BindGv_XzSchoolCache)',
      );
   ...
   **/
  public async BindTab_XzSchool(
    divContainer: HTMLDivElement,
    arrXzSchoolExObjLst: Array<clsXzSchoolENEx>,
  ) {
   ...
   **/
  public async BindTab_XzSchool4Func(
    divContainer: HTMLDivElement,
    arrXzSchoolExObjLst: Array<clsXzSchoolENEx>,
  ) {
  ...
  public async ExtendFldFuncMap(
    ...
    for (const objDataColumn of arrDataColumn) {
      ...
      for (const objInFor of arrXzSchoolExObjLst) {
        await XzSchoolEx_FuncMapByFldName(objDataColumn.fldName, objInFor);
      }
    }
  }
  ...
  public async BindGv_XzSchool4Func(divList: HTMLDivElement) {
    ...
    try {
      ...
      arrXzSchoolExObjLst = await XzSchoolEx_GetObjExLstByPagerCache(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error(strMsg);
      alert(strMsg);
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortXzSchoolBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (clsXzSchoolENEx.hasProperty(sortColumnKey)) {
      // 调用 SortColumn 函数
