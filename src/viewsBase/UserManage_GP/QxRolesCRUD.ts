...
import {
  QxPrjId_Local,
  CombineQxRolesConditionObj4ExportExcel,
  CombineQxRolesConditionObj,
...
import {
  QxRoles_GetRecCountByCondCache,
  QxRoles_GetSubObjLstCache,
  QxRoles_DelRecordAsync,
  QxRoles_ReFreshCache,
  QxRoles_GetObjByRoleIdAsync,
  QxRoles_FuncMapByFldName,
  QxRoles_GetObjExLstByPagerCache,
  QxRoles_GetObjLstByRoleIdLstAsync,
  QxRoles_GetMaxStrIdByPrefixAsync,
  QxRoles_AddNewRecordAsync,
  QxRoles_UpdateRecordAsync,
  QxRoles_DelQxRolessAsync,
...
export abstract class QxRolesCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortQxRolesBy == '')
        viewVarSet.sortQxRolesBy = `${clsQxRolesEN.con_RoleId} Asc`;
   ...
   **/
  public CombineData(
    arrQxRolesObjLst: Array<clsQxRolesEN>,
    ...
    const strFileName = Format('角色({0})导出.xlsx', clsQxRolesEN._CurrTabName);
  ...
  public CombineData4Func(
    arrQxRolesExObjLst: Array<clsQxRolesENEx>,
    arrDataColumn: Array<clsDataColumn>,
  ): ExportExcelData {
  ...
  public async ExportExcel_QxRoles4Func(): Promise<ExportExcelData> {
    ...
    if (viewVarSet.sortQxRolesBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortQxRolesBy)为空,请检查!(In BindGv_QxRolesCache)',
      );
   ...
   **/
  public async BindTab_QxRoles4Func(
    divContainer: HTMLDivElement,
    arrQxRolesExObjLst: Array<clsQxRolesENEx>,
  ) {
