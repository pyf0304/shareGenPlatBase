...
import { qxRoleMenusCache } from '@/viewsShare/MenuManage_GP/QxRoleMenusVueShare';
import { isFuncMapCache } from '@/viewsShare/MenuManage_GP/QxRoleMenusVueShare';
...
export async function QxRoleMenusEx_GetObjExLstByPagerCache(
  ...
  const arrQxRoleMenusExObjLst = arrQxRoleMenusObjLst.map((obj) => {
    ...
    if (qxRoleMenusCache[cacheKey]) {
      ...
      const newObj = QxRoleMenusEx_CopyToEx(obj);
  ...
  for (const newObj of arrNewObj) {
    for (const strFldName of Object.keys(isFuncMapCache)) {
      await QxRoleMenusEx_FuncMapByFldName(strFldName, newObj);
    }
  }
  ...
  if (
    ...
    for (const newObj of arrQxRoleMenusExObjLst) {
      await QxRoleMenusEx_FuncMapByFldName(objSortInfo.SortFld, newObj);
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      arrQxRoleMenusSel = arrQxRoleMenusSel.sort(
        QxRoleMenusEx_SortFunByKey(objSortInfo.SortFld, objSortInfo.SortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空,就调用排序函数
      arrQxRoleMenusSel = arrQxRoleMenusSel.sort(objPagerPara.sortFun);
