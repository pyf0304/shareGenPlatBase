/**
 * 类名:QxPrjMenus_Edit(界面:QxPrjMenusCRUD,00140040)
 * 表名:QxPrjMenus(00140004)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/13 16:04:16
 * 生成者:
 工程名称:统一平台(0014)
...
export abstract class QxPrjMenus_Edit {
  ...
  public HideDialog_QxPrjMenus() {
    if (QxPrjMenus_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      refQxPrjMenus_Edit.value.hideDialog();
    }
  }
  ...
  public async ShowDialog_QxPrjMenus(strOp: string): Promise<boolean> {
    ...
    if (QxPrjMenus_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refQxPrjMenus_Edit.value == null) {
        const strMsg = Format(
      ...
      await refQxPrjMenus_Edit.value.showDialog(this);
    }
   ...
   **/
  public async btnUpdateRecordInTab_Click(strMenuId: string) {
   ...
   **/
  public async btnUpdateRecord_Click(strMenuId: string) {
    ...
    try {
      ...
      const update = await this.UpdateRecord(strMenuId);
      if (update == false) {
  ...
  public async AddNewRecord() {
    ...
    refQxPrjMenus_Edit.value.Clear();
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    let objQxPrjMenusEN;
    try {
      objQxPrjMenusEN = await refQxPrjMenus_Edit.value.GetEditDataQxPrjMenusObj();
    } catch (e) {
      const strMsg = Format(
        '从界面获取数据不成功,{0}.(in {1}.{2})',
        e,
    ...
    try {
      ...
      if (returnBool == true) {
        const strInfo = `添加[工程菜单(QxPrjMenus)]记录成功!`;
        //显示信息框
  ...
  public async ShowData(strMenuId: string) {
    ...
    //3、用提供的关键字初始化一个类对象；
    refQxPrjMenus_Edit.value.ShowDataFromQxPrjMenusObj(objQxPrjMenusEN);
  }
  ...
  public async UpdateRecord(strMenuId: string): Promise<boolean> {
    ...
    try {
      ...
      await refQxPrjMenus_Edit.value.ShowDataFromQxPrjMenusObj(objQxPrjMenusEN);
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    const objQxPrjMenusEN = await refQxPrjMenus_Edit.value.GetEditDataQxPrjMenusObj();
  ...
  public async EditRecordExSave(): Promise<boolean> {
    ...
    try {
      ...
      const returnBool = await QxPrjMenus_EditRecordExAsync(objQxPrjMenusEN);
      if (returnBool == true) {
        const qxPrjMenusStore = useQxPrjMenusStore();
        await qxPrjMenusStore.delObj(objQxPrjMenusEN.menuId);
      }
  ...
  public set btnCancelQxPrjMenus(value: string) {
    refQxPrjMenus_Edit.value.strCancelButtonText = value;
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitQxPrjMenus(): string {
    const strValue = refQxPrjMenus_Edit.value.strSubmitButtonText;
    return strValue;
  }
  /**
  ...
  public set btnSubmitQxPrjMenus(value: string) {
    refQxPrjMenus_Edit.value.strSubmitButtonText = value;
  }
}
