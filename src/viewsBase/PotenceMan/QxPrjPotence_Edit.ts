 ...
 /**
 * 类名:QxPrjPotence_Edit(界面:QxPrjPotenceCRUD,00140042)
 * 表名:QxPrjPotence(00140005)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/13 16:04:31
 * 生成者:
 工程名称:统一平台(0014)
...
import { QxPrjId_Local,QxPrjPotence_DeleteKeyIdCache,divVarSet,refQxPrjPotence_Edit } from "@/viewsShare/PotenceMan/QxPrjPotenceVueShare";
import { QxPrjPotence_GetMaxStrIdByPrefixAsync,QxPrjPotence_CheckPropertyNew,QxPrjPotence_AddNewRecordWithMaxIdAsync,QxPrjPotence_ReFreshCache,QxPrjPotence_GetUniCondStr,QxPrjPotence_IsExistRecordAsync,QxPrjPotence_GetUniCondStr4Update,QxPrjPotence_IsExistAsync,QxPrjPotence_GetObjByPotenceIdAsync,QxPrjPotence_CheckProperty4Update,QxPrjPotence_UpdateRecordAsync,QxPrjPotence_EditRecordExAsync } from "@/ts/L3ForWApi/PotenceMan/clsQxPrjPotenceWApi";
...
{
 refQxPrjPotence_Edit.value.hideDialog();
}
}
...
if (QxPrjPotence_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refQxPrjPotence_Edit.value == null)
{
const strMsg = Format(
...
await refQxPrjPotence_Edit.value.showDialog(this);
}
 ...
 **/
public async btnUpdateRecordInTab_Click(strPotenceId: string) {
...
const bolIsSuccess = await this.ShowDialog_QxPrjPotence(this.opType);
if (bolIsSuccess == false) return;
this.UpdateRecord(strPotenceId);
}
catch (e)
{
 ...
 **/
public async btnUpdateRecord_Click(strPotenceId: string) {
...
const update = await this.UpdateRecord(strPotenceId);
if (update == false)
{
...
if (this.iShowList != null) this.iShowList.BindGvCache(clsQxPrjPotenceEN._CurrTabName, returnKeyId);
}
}
else
{
...
if (this.iShowList != null) this.iShowList.BindGvCache(clsQxPrjPotenceEN._CurrTabName, this.keyId);
}
}
break;
case "确认修改":
...
refQxPrjPotence_Edit.value.Clear();
//wucQxPrjPotenceB1.potenceId = QxPrjPotenceGetMaxStrId_S();
try
{
const returnString = await QxPrjPotence_GetMaxStrIdByPrefixAsync(QxPrjId_Local.value);
if (returnString == "")
{
...
let objQxPrjPotenceEN;
try
{
objQxPrjPotenceEN = await refQxPrjPotence_Edit.value.GetEditDataQxPrjPotenceObj();
}
catch(e)
{
...
const strInfo = `添加[工程权限(QxPrjPotence)]记录成功!`;
//显示信息框
...
console.error(strMsg);
        ...
        //3、用提供的关键字初始化一个类对象；
refQxPrjPotence_Edit.value.ShowDataFromQxPrjPotenceObj(objQxPrjPotenceEN);
}
...
await refQxPrjPotence_Edit.value.ShowDataFromQxPrjPotenceObj(objQxPrjPotenceEN);
...
const objQxPrjPotenceEN = await refQxPrjPotence_Edit.value.GetEditDataQxPrjPotenceObj();
...
{
QxPrjPotence_ReFreshCache(QxPrjId_Local.value);
QxPrjPotence_DeleteKeyIdCache(QxPrjId_Local.value, this.keyId);
}
return returnBool;
}
catch(e)
...
const returnBool = await QxPrjPotence_EditRecordExAsync(objQxPrjPotenceEN);
if (returnBool == true)
{
QxPrjPotence_ReFreshCache(QxPrjId_Local.value);
}
...
refQxPrjPotence_Edit.value.strCancelButtonText = value;
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmitQxPrjPotence():string {
const strValue = refQxPrjPotence_Edit.value.strSubmitButtonText;
return strValue;
}
 /**
...
refQxPrjPotence_Edit.value.strSubmitButtonText = value;
}
}