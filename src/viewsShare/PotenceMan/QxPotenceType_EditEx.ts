/**
 * 类名:QxPotenceType_EditEx(界面:QxPotenceTypeCRUD)
...
/* QxPotenceType_EditEx 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_EditCSEx_TS4TypeScript:GeneCode)
*/
export default class QxPotenceType_EditEx extends QxPotenceType_Edit {
  /**
 ...
 **/
  public static btnEdit_Click(strCommandName: string, strKeyId: string) {
    ...
    const objPage: QxPotenceType_EditEx = <QxPotenceType_EditEx>(
      QxPotenceType_Edit.GetPageEditObj('QxPotenceType_EditEx')
    );
    if (objPage == null) {
      const strMsg = `从预存编辑类获取关键字:[QxPotenceType_EditEx]的对象为空，请在调用编辑类(父类)的定义一下！`;
