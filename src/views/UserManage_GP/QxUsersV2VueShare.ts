...
if ( userName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxUsersV2EN.con_UserName, userName_q.value);
}
if ( nickName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxUsersV2EN.con_nickName, nickName_q.value);
}
if ( email_q.value != "")
{
...
{
const strMsg:string = Format("在组合查询条件(CombineQxUsersV2Condition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
...
{
const strMsg:string = Format("在组合查询条件对象(CombineQxUsersV2ConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
...
{
const strMsg:string = Format("在组合导出Excel条件对象(CombineQxUsersV2ConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objQxUsersV2Cond.whereCond = strWhereCond;
return objQxUsersV2Cond;
