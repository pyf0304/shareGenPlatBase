...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxUserIdentityEN.con_IdentityId, identityId_q.value);
}
if ( identityDesc_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxUserIdentityEN.con_IdentityDesc, identityDesc_q.value);
}

}
catch(objException)
{
const strMsg:string = Format("在组合查询条件(CombineQxUserIdentityCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxUserIdentityEN.con_IdentityId, identityId_q.value);
objQxUserIdentityCond.SetCondFldValue(clsQxUserIdentityEN.con_IdentityId, identityId_q.value, "like");
}
if ( identityDesc_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxUserIdentityEN.con_IdentityDesc, identityDesc_q.value);
objQxUserIdentityCond.SetCondFldValue(clsQxUserIdentityEN.con_IdentityDesc, identityDesc_q.value, "like");
}

}
catch(objException)
{
const strMsg:string = Format("在组合查询条件对象(CombineQxUserIdentityConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
export const CombineQxUserIdentityConditionObj4ExportExcel = async ():Promise<ConditionCollection> => 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
const objQxUserIdentityCond = new ConditionCollection();
...
{
const strMsg:string = Format("在组合导出Excel条件对象(CombineQxUserIdentityConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objQxUserIdentityCond.whereCond = strWhereCond;
return objQxUserIdentityCond;
...
if (
IsNullOrEmpty(strIdentityId) == false )
{
// 使用 delete 删除特定的键
