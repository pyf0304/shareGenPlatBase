 ...
 * @returns 对象列表
*/
export  async function QxRoles_GetObjExLstByPagerCache(objPagerPara: stuPagerPara , strQxPrjId:string):Promise<Array<clsQxRolesENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
...
const intEnd = intStart + objPagerPara.pageSize;
if (objPagerPara.orderBy != null && objPagerPara.orderBy.length>0) {
arrQxRolesSel = arrQxRolesSel.sort(QxRoles_SortFunByExKey(objSortInfo.SortFld, objSortInfo.SortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrQxRolesSel = arrQxRolesSel.sort(objPagerPara.sortFun);
...
{
const strMsg = Format("(errid:Watl001294)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxRoles_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objQxRolesENT;
}
...
{
const strMsg = Format("(errid:Watl001400)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxRoles_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
{
const strMsg = Format("(errid:Watl001401)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxRoles_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
{
const strMsg = Format("(errid:Watl001402)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxRoles_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
