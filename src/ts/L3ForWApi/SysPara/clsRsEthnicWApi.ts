...
export async function RsEthnic_func(
  ...
  if (clsRsEthnicEN._AttributeName.indexOf(strOutFldName) == -1) {
    const strMsg = Format(
      '输出字段名:[{0}]不正确,不在输出字段范围之内!({1})',
      strOutFldName,
      clsRsEthnicEN._AttributeName.join(','),
    );
    console.error(strMsg);
    throw new Error(strMsg);
  }
 ...
 **/
export async function RsEthnic_GetFldValueAsync(
  strFldName: string,
  strWhereCond: string,
): Promise<Array<string>> {
 ...
 **/
export async function RsEthnic_AddNewObjSave(
  objRsEthnicEN: clsRsEthnicEN,
