/**
 * 类名:QxRoleMenus_EditEx(界面:QxRoleMenusCRUD,00140029)
...
import { QxRoleMenus_Edit } from '@/viewsBase/MenuManage_GP/QxRoleMenus_Edit';
/* QxRoleMenus_EditEx 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_EditCSEx_TS4TypeScript:GeneCode)
*/
export default class QxRoleMenus_EditEx extends QxRoleMenus_Edit {
  /**
 ...
 **/
  public static btnEdit_Click(strCommandName: string, strKeyId: string) {
    ...
    const objPage: QxRoleMenus_EditEx = <QxRoleMenus_EditEx>(
      QxRoleMenus_Edit.GetPageEditObj('QxRoleMenus_EditEx')
    );
    if (objPage == null) {
      const strMsg = `从预存编辑类获取关键字:[QxRoleMenus_EditEx]的对象为空，请在调用编辑类(父类)的定义一下！`;
