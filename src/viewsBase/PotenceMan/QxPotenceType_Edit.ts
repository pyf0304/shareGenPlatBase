 ...
 /**
 * 类名:QxPotenceType_Edit(界面:QxPotenceTypeCRUD,00140041)
 * 表名:QxPotenceType(00140003)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/13 16:04:24
 * 生成者:
 工程名称:统一平台(0014)
...
import { QxPrjId_Local,QxPotenceType_DeleteKeyIdCache,divVarSet,refQxPotenceType_Edit } from "@/viewsShare/PotenceMan/QxPotenceTypeVueShare";
import { QxPotenceType_GetMaxStrIdByPrefixAsync,QxPotenceType_CheckPropertyNew,QxPotenceType_AddNewRecordWithMaxIdAsync,QxPotenceType_ReFreshCache,QxPotenceType_GetUniCondStr,QxPotenceType_IsExistRecordAsync,QxPotenceType_GetUniCondStr4Update,QxPotenceType_IsExistAsync,QxPotenceType_GetObjByPotenceTypeIdAsync,QxPotenceType_CheckProperty4Update,QxPotenceType_UpdateRecordAsync,QxPotenceType_EditRecordExAsync } from "@/ts/L3ForWApi/PotenceMan/clsQxPotenceTypeWApi";
...
{
 refQxPotenceType_Edit.value.hideDialog();
}
}
...
if (QxPotenceType_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refQxPotenceType_Edit.value == null)
{
const strMsg = Format(
...
await refQxPotenceType_Edit.value.showDialog(this);
}
 ...
 **/
public async btnUpdateRecordInTab_Click(strPotenceTypeId: string) {
...
const bolIsSuccess = await this.ShowDialog_QxPotenceType(this.opType);
if (bolIsSuccess == false) return;
this.UpdateRecord(strPotenceTypeId);
}
catch (e)
{
 ...
 **/
public async btnUpdateRecord_Click(strPotenceTypeId: string) {
...
const update = await this.UpdateRecord(strPotenceTypeId);
if (update == false)
{
...
if (this.iShowList != null) this.iShowList.BindGvCache(clsQxPotenceTypeEN._CurrTabName, returnKeyId);
}
}
else
{
...
if (this.iShowList != null) this.iShowList.BindGvCache(clsQxPotenceTypeEN._CurrTabName, this.keyId);
}
}
break;
case "确认修改":
...
refQxPotenceType_Edit.value.Clear();
//wucQxPotenceTypeB1.potenceTypeId = QxPotenceTypeGetMaxStrId_S();
try
{
const returnString = await QxPotenceType_GetMaxStrIdByPrefixAsync(QxPrjId_Local.value);
if (returnString == "")
{
...
let objQxPotenceTypeEN;
try
{
objQxPotenceTypeEN = await refQxPotenceType_Edit.value.GetEditDataQxPotenceTypeObj();
}
catch(e)
{
...
const strInfo = `添加[权限类型(QxPotenceType)]记录成功!`;
//显示信息框
...
console.error(strMsg);
        ...
        try {
const objQxPotenceTypeENConst = await QxPotenceType_GetObjByPotenceTypeIdAsync(strPotenceTypeId);
...
console.error(strMsg);
        ...
        //3、用提供的关键字初始化一个类对象；
refQxPotenceType_Edit.value.ShowDataFromQxPotenceTypeObj(objQxPotenceTypeEN);
}
...
await refQxPotenceType_Edit.value.ShowDataFromQxPotenceTypeObj(objQxPotenceTypeEN);
...
const objQxPotenceTypeEN = await refQxPotenceType_Edit.value.GetEditDataQxPotenceTypeObj();
...
{
QxPotenceType_ReFreshCache();
QxPotenceType_DeleteKeyIdCache(this.keyId);
}
return returnBool;
}
catch(e)
...
const returnBool = await QxPotenceType_EditRecordExAsync(objQxPotenceTypeEN);
if (returnBool == true)
{
QxPotenceType_ReFreshCache();
}
...
refQxPotenceType_Edit.value.strCancelButtonText = value;
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmitQxPotenceType():string {
const strValue = refQxPotenceType_Edit.value.strSubmitButtonText;
return strValue;
}
 /**
...
refQxPotenceType_Edit.value.strSubmitButtonText = value;
}
}