/**
 * 类名:QxPrjPotence_EditEx(界面:QxPrjPotenceCRUD)
...
/* QxPrjPotence_EditEx 的摘要说明。其中Q代表查询,U代表修改
  (AutoGCLib.Vue_ViewScript_EditCSEx_TS4TypeScript:GeneCode)
*/
export default class QxPrjPotence_EditEx extends QxPrjPotence_Edit {
  /**
 ...
 **/
  public static btnEdit_Click(strCommandName: string, strKeyId: string) {
    ...
    const objPage: QxPrjPotence_EditEx = <QxPrjPotence_EditEx>(
      QxPrjPotence_Edit.GetPageEditObj('QxPrjPotence_EditEx')
    );
    if (objPage == null) {
      const strMsg = `从预存编辑类获取关键字:[QxPrjPotence_EditEx]的对象为空，请在调用编辑类(父类)的定义一下！`;
