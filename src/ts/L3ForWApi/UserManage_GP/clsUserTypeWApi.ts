...
let strUrl = GetWebApiUrl_GP(userType_Controller, strAction);
...
{
const strInfo = `添加[UserType(UserType)]记录不成功!`;
//显示信息框
throw(strInfo);
}
...
BindDdl_ObjLstInDivObj(objDiv, strDdlName, arrObjLstSel, clsUserTypeEN.con_UserTypeId, clsUserTypeEN.con_UserTypeName, "选用户类型...");
}
...
{
 throw new Error(`(errid:Watl000411)字段[用户类型名]不能为空(In UserType)!(clsUserTypeBL:CheckPropertyNew0)`);
}
//检查字段长度, 若字符型字段长度超出规定的长度,即非法!
if (IsNullOrEmpty(pobjUserTypeEN.userTypeId) == false && GetStrLen(pobjUserTypeEN.userTypeId) > 2)
{
 throw new Error(`(errid:Watl000413)字段[用户类型Id(userTypeId)]的长度不能超过2(In UserType(UserType))!值:${pobjUserTypeEN.userTypeId}(clsUserTypeBL:CheckPropertyNew)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeName) == false && GetStrLen(pobjUserTypeEN.userTypeName) > 20)
{
 throw new Error(`(errid:Watl000413)字段[用户类型名(userTypeName)]的长度不能超过20(In UserType(UserType))!值:${pobjUserTypeEN.userTypeName}(clsUserTypeBL:CheckPropertyNew)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeNameSim) == false && GetStrLen(pobjUserTypeEN.userTypeNameSim) > 20)
{
 throw new Error(`(errid:Watl000413)字段[用户类型简称(userTypeNameSim)]的长度不能超过20(In UserType(UserType))!值:${pobjUserTypeEN.userTypeNameSim}(clsUserTypeBL:CheckPropertyNew)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeENName) == false && GetStrLen(pobjUserTypeEN.userTypeENName) > 50)
{
 throw new Error(`(errid:Watl000413)字段[用户类型英文名(userTypeENName)]的长度不能超过50(In UserType(UserType))!值:${pobjUserTypeEN.userTypeENName}(clsUserTypeBL:CheckPropertyNew)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.memo) == false && GetStrLen(pobjUserTypeEN.memo) > 1000)
{
 throw new Error(`(errid:Watl000413)字段[备注(memo)]的长度不能超过1000(In UserType(UserType))!值:${pobjUserTypeEN.memo}(clsUserTypeBL:CheckPropertyNew)`);
}
//检查字段的数据类型是否正确
...
{
 throw new Error(`(errid:Watl000414)字段[用户类型Id(userTypeId)]的值:[${pobjUserTypeEN.userTypeId}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckPropertyNew0)`);
}
...
{
 throw new Error(`(errid:Watl000414)字段[用户类型名(userTypeName)]的值:[${pobjUserTypeEN.userTypeName}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckPropertyNew0)`);
}
...
{
 throw new Error(`(errid:Watl000414)字段[用户类型简称(userTypeNameSim)]的值:[${pobjUserTypeEN.userTypeNameSim}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckPropertyNew0)`);
}
...
{
 throw new Error(`(errid:Watl000414)字段[用户类型英文名(userTypeENName)]的值:[${pobjUserTypeEN.userTypeENName}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckPropertyNew0)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.memo) == false && undefined !== pobjUserTypeEN.memo && tzDataType.isString(pobjUserTypeEN.memo) === false)
{
 throw new Error(`(errid:Watl000414)字段[备注(memo)]的值:[${pobjUserTypeEN.memo}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckPropertyNew0)`);
}
...
if (IsNullOrEmpty(pobjUserTypeEN.userTypeId) == false && GetStrLen(pobjUserTypeEN.userTypeId) > 2)
{
 throw new Error(`(errid:Watl000416)字段[用户类型Id(userTypeId)]的长度不能超过2(In UserType(UserType))!值:${pobjUserTypeEN.userTypeId}(clsUserTypeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeName) == false && GetStrLen(pobjUserTypeEN.userTypeName) > 20)
{
 throw new Error(`(errid:Watl000416)字段[用户类型名(userTypeName)]的长度不能超过20(In UserType(UserType))!值:${pobjUserTypeEN.userTypeName}(clsUserTypeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeNameSim) == false && GetStrLen(pobjUserTypeEN.userTypeNameSim) > 20)
{
 throw new Error(`(errid:Watl000416)字段[用户类型简称(userTypeNameSim)]的长度不能超过20(In UserType(UserType))!值:${pobjUserTypeEN.userTypeNameSim}(clsUserTypeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeENName) == false && GetStrLen(pobjUserTypeEN.userTypeENName) > 50)
{
 throw new Error(`(errid:Watl000416)字段[用户类型英文名(userTypeENName)]的长度不能超过50(In UserType(UserType))!值:${pobjUserTypeEN.userTypeENName}(clsUserTypeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.memo) == false && GetStrLen(pobjUserTypeEN.memo) > 1000)
{
 throw new Error(`(errid:Watl000416)字段[备注(memo)]的长度不能超过1000(In UserType(UserType))!值:${pobjUserTypeEN.memo}(clsUserTypeBL:CheckProperty4Update)`);
}
//检查字段的数据类型是否正确
if (IsNullOrEmpty(pobjUserTypeEN.userTypeId) == false && undefined !== pobjUserTypeEN.userTypeId && tzDataType.isString(pobjUserTypeEN.userTypeId) === false)
{
 throw new Error(`(errid:Watl000417)字段[用户类型Id(userTypeId)]的值:[${pobjUserTypeEN.userTypeId}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeName) == false && undefined !== pobjUserTypeEN.userTypeName && tzDataType.isString(pobjUserTypeEN.userTypeName) === false)
{
 throw new Error(`(errid:Watl000417)字段[用户类型名(userTypeName)]的值:[${pobjUserTypeEN.userTypeName}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeNameSim) == false && undefined !== pobjUserTypeEN.userTypeNameSim && tzDataType.isString(pobjUserTypeEN.userTypeNameSim) === false)
{
 throw new Error(`(errid:Watl000417)字段[用户类型简称(userTypeNameSim)]的值:[${pobjUserTypeEN.userTypeNameSim}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.userTypeENName) == false && undefined !== pobjUserTypeEN.userTypeENName && tzDataType.isString(pobjUserTypeEN.userTypeENName) === false)
{
 throw new Error(`(errid:Watl000417)字段[用户类型英文名(userTypeENName)]的值:[${pobjUserTypeEN.userTypeENName}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjUserTypeEN.memo) == false && undefined !== pobjUserTypeEN.memo && tzDataType.isString(pobjUserTypeEN.memo) === false)
{
 throw new Error(`(errid:Watl000417)字段[备注(memo)]的值:[${pobjUserTypeEN.memo}], 非法,应该为字符型(In UserType(UserType))!(clsUserTypeBL:CheckProperty4Update)`);
}
//检查主键是否为Null或者空!
if (IsNullOrEmpty(pobjUserTypeEN.userTypeId) === true 
 || pobjUserTypeEN.userTypeId.toString()  ===  "0" )
{
 throw new Error(`(errid:Watl000064)字段[用户类型Id]不能为空(In UserType)!(clsUserTypeBL:CheckProperty4Update)`);
}
//检查外键, 作为外键应该和主键的字段长度是一样的, 若不一样,即非法!
