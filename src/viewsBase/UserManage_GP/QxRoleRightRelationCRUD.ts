/**
 * 类名:QxRoleRightRelationCRUD(界面:QxRoleRightRelationCRUD,00140044)
 * 表名:QxRoleRightRelation(00140118)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/13 16:04:42
 * 生成者:
 工程名称:统一平台(0014)
...
import {
  CombineQxRoleRightRelationConditionObj4ExportExcel,
  QxPrjId_Local,
  CombineQxRoleRightRelationConditionObj,
  divVarSet,
  viewVarSet,
  dataColumn,
  BindTabByList,
...
import {
  QxRoleRightRelation_GetRecCountByCondCache,
  QxRoleRightRelation_GetSubObjLstCache,
  QxRoleRightRelation_DelRecordAsync,
  QxRoleRightRelation_ReFreshCache,
  QxRoleRightRelation_GetObjBymIdAsync,
  QxRoleRightRelation_FuncMapByFldName,
  QxRoleRightRelation_GetObjExLstByPagerCache,
...
import { QxRoleRightRelationEx_CopyToEx } from '@/ts/L3ForWApiEx/UserManage_GP/clsQxRoleRightRelationExWApi';
import {
...
export abstract class QxRoleRightRelationCRUD implements clsOperateList {
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      if (viewVarSet.sortQxRoleRightRelationBy == '')
        viewVarSet.sortQxRoleRightRelationBy = `${clsQxRoleRightRelationEN.con_mId} Asc`;
   ...
   **/
  public async ExportExcel_QxRoleRightRelation4Func(): Promise<ExportExcelData> {
    const strThisFuncName = this.ExportExcel_QxRoleRightRelation4Func.name;
    if (viewVarSet.sortQxRoleRightRelationBy == null) {
      const strMsg = Format(
    ...
    const objQxRoleRightRelationCond = await CombineQxRoleRightRelationConditionObj4ExportExcel();
    let arrQxRoleRightRelationExObjLst: Array<clsQxRoleRightRelationENEx> = [];
    try {
      this.recCount = await QxRoleRightRelation_GetRecCountByCondCache(objQxRoleRightRelationCond);
      if (this.recCount == 0) {
        const strMsg = Format(
          '在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!',
          objQxRoleRightRelationCond.whereCond,
        );
        console.error('Error: ', strMsg);
        //console.trace();
        alert(strMsg);
      ...
      const arrQxRoleRightRelationObjLst = await QxRoleRightRelation_GetSubObjLstCache(
        objQxRoleRightRelationCond,
      );
    ...
    try {
      ...
      arrQxRoleRightRelationExObjLst = arrQxRoleRightRelationExObjLst.sort(this.SortFunExportExcel);
      return this.CombineData(arrQxRoleRightRelationExObjLst, arrDataColumn);
  ...
  public async BindTab_QxRoleRightRelation(
    ...
    if (refQxRoleRightRelation_List.value != null) {
      dataColumn.value = arrDataColumn;
      await BindTabByList(arrQxRoleRightRelationExObjLst, this.dispAllErrMsg_q);
    } else {
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_QxRoleRightRelation函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async BindTab_QxRoleRightRelation4Func(
    ...
    if (refQxRoleRightRelation_List.value != null) {
      ...
      const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
      if (divDataLst == null) {
        alert('在BindTab_QxRoleRightRelation4Func函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async ExtendFldFuncMap(
    ...
    for (const objDataColumn of arrDataColumn) {
      ...
      for (const objInFor of arrQxRoleRightRelationExObjLst) {
        ...
        objInFor.qxPrjId = QxPrjId_Local.value;
        await QxRoleRightRelation_FuncMapByFldName(objDataColumn.fldName, objInFor);
      }
    }
  }
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_QxRoleRightRelation4Func(this.listPara.listDiv);
  }
  ...
  public async BindGv_QxRoleRightRelation4Func(divList: HTMLDivElement) {
    ...
    const objQxRoleRightRelationCond = await CombineQxRoleRightRelationConditionObj();
    const strWhereCond = JSON.stringify(objQxRoleRightRelationCond);
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrQxRoleRightRelationExObjLst, true);
        return;
      }
      ...
      if (QxRoleRightRelationCRUD.sortFunStatic != undefined) {
        strSortFun = QxRoleRightRelationCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
      }
      const objPagerPara: stuPagerPara = {
        pageIndex: intCurrPageIndex,
        pageSize: this.pageSize,
        whereCond: strWhereCond,
        conditionCollection: objQxRoleRightRelationCond,
    ...
    try {
      ...
      //console.log("完成BindGv_QxRoleRightRelation4Func!");
    } catch (e) {
      const strMsg = `绑定${this.thisTabName}对象列表不成功, ${e}.(in ${this.constructor.name}.${strThisFuncName})`;
      console.error(strMsg);
      alert(strMsg);
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(
      ...
      viewVarSet.sortQxRoleRightRelationBy,
      strSortExpress,
    );
    // 将 sortBy 按空格分成两部分
    const [sortColumnKey, sortDirection] = sortBy.split(' ');
    if (Object.prototype.hasOwnProperty.call(clsQxRoleRightRelationENEx, sortColumnKey)) {
      // 调用 SortColumn 函数
      this.SortColumn(sortColumnKey, sortDirection);
      return;
    }
    viewVarSet.sortQxRoleRightRelationBy = sortBy;
