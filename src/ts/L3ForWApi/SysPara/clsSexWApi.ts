...
export async function Sex_func(strInFldName: string, strOutFldName: string, strInValue: string) {
  ...
  if (clsSexEN._AttributeName.indexOf(strOutFldName) == -1) {
    const strMsg = Format(
      '输出字段名:[{0}]不正确,不在输出字段范围之内!({1})',
      strOutFldName,
      clsSexEN._AttributeName.join(','),
    );
    console.error(strMsg);
    throw new Error(strMsg);
  }
 ...
 **/
export async function Sex_GetFldValueAsync(
  strFldName: string,
  strWhereCond: string,
): Promise<Array<string>> {
...
export function Sex_CheckProperty4Update(pobjSexEN: clsSexEN) {
  ...
  //检查主键是否为Null或者空!
  if (IsNullOrEmpty(pobjSexEN.idSex) === true || pobjSexEN.idSex.toString() === '0') {
    throw new Error(
