...
if ( roleName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxRolesV2EN.con_RoleName, roleName_q.value);
}
...
{
const strMsg:string = Format("在组合查询条件(CombineQxRolesV2Condition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
...
{
const strMsg:string = Format("在组合查询条件对象(CombineQxRolesV2ConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
...
{
const strMsg:string = Format("在组合导出Excel条件对象(CombineQxRolesV2ConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objQxRolesV2Cond.whereCond = strWhereCond;
return objQxRolesV2Cond;
