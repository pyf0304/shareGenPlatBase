...
const strUrl = GetWebApiUrl_GP(cacheMode_Controller, strAction);
...
{
const arrCacheModeObjLstCache = await CacheMode_GetObjLstCache();
const obj = arrCacheModeObjLstCache.find(x => 
x.cacheModeId == objCacheMode.cacheModeId);
if (obj != null)
{
...
if (clsCacheModeEN._AttributeName.indexOf(strOutFldName) == -1)
{
const strMsg = Format("输出字段名:[{0}]不正确,不在输出字段范围之内!({1})",
strOutFldName, clsCacheModeEN._AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (IsNullOrEmpty(clsCacheModeEN._WhereFormat) == false)
{
strWhereCond = clsCacheModeEN._WhereFormat;
}
if (IsNullOrEmpty(clsCacheModeEN._CacheAddiCondition) == false)
{
strWhereCond += Format(" and {0}", clsCacheModeEN._CacheAddiCondition);
}
if (strKey == "")
{
console.error("关键字为空!不正确");
...
switch (clsCacheModeEN._CacheModeId)
{
...
let strUrl = GetWebApiUrl_GP(cacheMode_Controller, strAction);
...
const bolIsExist = await CacheMode_IsExistAsync(objCacheModeEN.cacheModeId);
if (bolIsExist == true)
{
const strMsg = Format("添加记录时,关键字：{0}已经存在!", objCacheModeEN.cacheModeId);
console.error(strMsg);
throw(strMsg);
}
returnBool = await CacheMode_AddNewRecordAsync(objCacheModeEN);
if (returnBool == true)
{
...
return { keyword: objCacheModeEN.cacheModeId, success: returnBool };//一定要有一个返回值,否则会出错!
}
catch(e)
{
...
*/
export  async function CacheMode_BindDdl_CacheModeIdInDivCache(objDiv: HTMLDivElement, strDdlName: string )
{
...
//为数据源于表的下拉框设置内容
//console.log("开始：BindDdl_CacheModeIdInDivCache");
const arrObjLstSel = await CacheMode_GetObjLstCache();
if (arrObjLstSel == null) return;
BindDdl_ObjLstInDivObj(objDiv, strDdlName, arrObjLstSel, clsCacheModeEN.con_CacheModeId, clsCacheModeEN.con_CacheModeName, "选缓存方式...");
}
...
*/
export  async function CacheMode_GetArrCacheMode()
{
...
{
 throw new Error(`(errid:Watl000411)字段[CacheModeName]不能为空(In 缓存方式)!(clsCacheModeBL:CheckPropertyNew0)`);
}
//检查字段长度, 若字符型字段长度超出规定的长度,即非法!
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeId) == false && GetStrLen(pobjCacheModeEN.cacheModeId) > 2)
{
 throw new Error(`(errid:Watl000413)字段[CacheModeId(cacheModeId)]的长度不能超过2(In 缓存方式(CacheMode))!值:${pobjCacheModeEN.cacheModeId}(clsCacheModeBL:CheckPropertyNew)`);
}
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeName) == false && GetStrLen(pobjCacheModeEN.cacheModeName) > 50)
{
 throw new Error(`(errid:Watl000413)字段[CacheModeName(cacheModeName)]的长度不能超过50(In 缓存方式(CacheMode))!值:${pobjCacheModeEN.cacheModeName}(clsCacheModeBL:CheckPropertyNew)`);
}
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeENName) == false && GetStrLen(pobjCacheModeEN.cacheModeENName) > 50)
{
 throw new Error(`(errid:Watl000413)字段[CacheModeENName(cacheModeENName)]的长度不能超过50(In 缓存方式(CacheMode))!值:${pobjCacheModeEN.cacheModeENName}(clsCacheModeBL:CheckPropertyNew)`);
}
...
{
 throw new Error(`(errid:Watl000414)字段[CacheModeId(cacheModeId)]的值:[${pobjCacheModeEN.cacheModeId}], 非法,应该为字符型(In 缓存方式(CacheMode))!(clsCacheModeBL:CheckPropertyNew0)`);
}
...
{
 throw new Error(`(errid:Watl000414)字段[CacheModeName(cacheModeName)]的值:[${pobjCacheModeEN.cacheModeName}], 非法,应该为字符型(In 缓存方式(CacheMode))!(clsCacheModeBL:CheckPropertyNew0)`);
}
...
{
 throw new Error(`(errid:Watl000414)字段[CacheModeENName(cacheModeENName)]的值:[${pobjCacheModeEN.cacheModeENName}], 非法,应该为字符型(In 缓存方式(CacheMode))!(clsCacheModeBL:CheckPropertyNew0)`);
}
...
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeId) == false && GetStrLen(pobjCacheModeEN.cacheModeId) > 2)
{
 throw new Error(`(errid:Watl000416)字段[CacheModeId(cacheModeId)]的长度不能超过2(In 缓存方式(CacheMode))!值:${pobjCacheModeEN.cacheModeId}(clsCacheModeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeName) == false && GetStrLen(pobjCacheModeEN.cacheModeName) > 50)
{
 throw new Error(`(errid:Watl000416)字段[CacheModeName(cacheModeName)]的长度不能超过50(In 缓存方式(CacheMode))!值:${pobjCacheModeEN.cacheModeName}(clsCacheModeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeENName) == false && GetStrLen(pobjCacheModeEN.cacheModeENName) > 50)
{
 throw new Error(`(errid:Watl000416)字段[CacheModeENName(cacheModeENName)]的长度不能超过50(In 缓存方式(CacheMode))!值:${pobjCacheModeEN.cacheModeENName}(clsCacheModeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjCacheModeEN.memo) == false && GetStrLen(pobjCacheModeEN.memo) > 1000)
{
...
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeId) == false && undefined !== pobjCacheModeEN.cacheModeId && tzDataType.isString(pobjCacheModeEN.cacheModeId) === false)
{
 throw new Error(`(errid:Watl000417)字段[CacheModeId(cacheModeId)]的值:[${pobjCacheModeEN.cacheModeId}], 非法,应该为字符型(In 缓存方式(CacheMode))!(clsCacheModeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeName) == false && undefined !== pobjCacheModeEN.cacheModeName && tzDataType.isString(pobjCacheModeEN.cacheModeName) === false)
{
 throw new Error(`(errid:Watl000417)字段[CacheModeName(cacheModeName)]的值:[${pobjCacheModeEN.cacheModeName}], 非法,应该为字符型(In 缓存方式(CacheMode))!(clsCacheModeBL:CheckProperty4Update)`);
}
if (IsNullOrEmpty(pobjCacheModeEN.cacheModeENName) == false && undefined !== pobjCacheModeEN.cacheModeENName && tzDataType.isString(pobjCacheModeEN.cacheModeENName) === false)
{
 throw new Error(`(errid:Watl000417)字段[CacheModeENName(cacheModeENName)]的值:[${pobjCacheModeEN.cacheModeENName}], 非法,应该为字符型(In 缓存方式(CacheMode))!(clsCacheModeBL:CheckProperty4Update)`);
}
if (null != pobjCacheModeEN.inUse && undefined !== pobjCacheModeEN.inUse && tzDataType.isBoolean(pobjCacheModeEN.inUse) === false)
{
...
{
 throw new Error(`(errid:Watl000064)字段[CacheModeId]不能为空(In 缓存方式)!(clsCacheModeBL:CheckProperty4Update)`);
}
//检查外键, 作为外键应该和主键的字段长度是一样的, 若不一样,即非法!
