
 /**
 * 类名:clsQxCmProjectENEx
 * 表名:QxCmProject(00140119)
 * 版本:2026.03.27.1(服务器:WIN-SRV103-116)
 * 日期:2026/03/29 07:03:10
 * 生成者:pyf
 工程名称:统一平台(0014)
 CM工程:统一平台前端(000057, 变量首字母小写)-WebApi函数集
 * 相关数据库:109.244.40.104,8433EduHigh_Jsie
 * PrjDataBaseId:0170
 模块中文名:系统管理(SysManage)
 * 框架-层名:实体扩展层(TS)(EntityLayerEx,0191)
 * 编程语言:TypeScript
 **/
 /**
 * QxCmProject(QxCmProject)
 * (AutoGCLib.EntityLayerEx4TypeScript:GeneCode)
 **/
import { clsQxCmProjectEN } from "@/ts/L0Entity/SysManage/clsQxCmProjectEN";

export class  clsQxCmProjectENEx extends clsQxCmProjectEN
{
//以下是属性变量

/**
 * 构造函数
 * (AutoGCLib.EntityLayerEx4TypeScript:GenClassConstructor1)
 **/
 constructor()
 {
 super();
 }

/**
 * 根据字段名获取对象中某字段的值.
 * (AutoGCLib.EntityLayerEx4TypeScript:Gen_ENEx_GetFldValue)
 * @param strFldName:字段名
 * @returns 字段值
*/
public GetFldValue(strFldName: string):any
{
let strValue;
switch (strFldName)
{
case "CtrlId":
return "";
case clsQxCmProjectENEx.con_UseStateName:
return this.useStateName;
default:
strValue = super.GetFldValue(strFldName);
return strValue;
}
}


 /**
 * 常量:"UseStateName"
 * (AutoGCLib.EntityLayerEx4TypeScript:Gen_ENEx_PropertyNameConst)
 */
 public static readonly con_UseStateName = "useStateName";    //使用状态名称

public useStateName = "";    //使用状态名称

/**
* 判断一个字符串是否是类的属性
* @param propName: 属性名
* @returns 是否是属性
*/
public static hasProperty(propName: string) : boolean {
//return propName in new clsQxCmProjectENEx();
const instance = new clsQxCmProjectENEx();
return Object.prototype.hasOwnProperty.call(instance, propName);
}
}