...
import { CombineQxUserStateConditionObj4ExportExcel,CombineQxUserStateConditionObj,divVarSet,viewVarSet,dataColumn,BindTabByList,refQxUserState_List } from "@/viewsShare/UserManage_GP/QxUserStateVueShare";
import { QxUserState_GetRecCountByCondCache,QxUserState_GetSubObjLstCache,QxUserState_DelRecordAsync,QxUserState_ReFreshCache,QxUserState_GetObjByUserStateIdAsync,QxUserState_FuncMapByFldName,QxUserState_GetObjExLstByPagerCache,QxUserState_GetObjLstByUserStateIdLstAsync,QxUserState_GetMaxStrIdAsync,QxUserState_AddNewRecordAsync,QxUserState_DelQxUserStatesAsync } from "@/ts/L3ForWApi/UserManage_GP/clsQxUserStateWApi";
...
if (viewVarSet.sortQxUserStateBy == '') viewVarSet.sortQxUserStateBy = `${ clsQxUserStateEN.con_UserStateId } Asc`;
 ...
 **/
public CombineData(arrQxUserStateObjLst: Array<clsQxUserStateEN>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
...
const strFileName = Format("用户状态({0})导出.xlsx",
 clsQxUserStateEN._CurrTabName);
...
{
const strMsg = Format("在显示列表时,排序字段(sortQxUserStateBy)为空,请检查!(In BindGv_QxUserStateCache)");
...
if (this.recCount == 0)
{
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objQxUserStateCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
 ...
 **/
public async BindTab_QxUserState(divContainer: HTMLDivElement, arrQxUserStateExObjLst: Array<clsQxUserStateENEx>) 
{
...
await BindTab(divDataLst, arrQxUserStateExObjLst, arrDataColumn,  clsQxUserStateEN.con_UserStateId, this); 
}
 ...
 **/
public async ExtendFldFuncMap(arrQxUserStateExObjLst: Array<clsQxUserStateENEx>, arrDataColumn: Array<clsDataColumn>) {
const arrFldName = clsQxUserStateEN.AttributeName;
for (const objDataColumn of arrDataColumn) {
if (IsNullOrEmpty(objDataColumn.fldName) == true) continue;
if (arrFldName.indexOf(objDataColumn.fldName) > -1) continue;
for (const objInFor of arrQxUserStateExObjLst) {
await QxUserState_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
lblMsg.innerHTML = Format("根据条件:[{0}]获取的对象列表数为0!", objQxUserStateCond.whereCond);
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objQxUserStateCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortQxUserStateBy, strSortExpress);
// 将 sortBy 按空格分成两部分
