...
export class  clsQxUserGroupPotenceEN extends clsGeneralTab
{
...
public static readonly _CurrTabName: string = "QxUserGroupPotence"; //当前表名,与该类相关的表名
public static readonly _KeyFldName: string = "mId"; //当前表中的关键字名称,与该类相关的表中关键字名
public static readonly _AttributeCount = 8;
public static readonly _AttributeName = ["mId", "userId", "groupId", "roleId", "qxPrjId", "potenceId", "relaMid", "memo"];
//以下是属性变量
 ...
 * (AutoGCLib.EntityLayer4TypeScript:Gen_EN_ClsProperty)
*/
 public SetGroupId (value: string)
{
if (value  != undefined)
{
 this.groupId = value;
    this.hmProperty["groupId"] = true;
   this.sfUpdFldSetStr = this.updFldString;
}
}
...
default:
strMsg = `字段名:[${strFldName}]在表对象:[QxUserGroupPotence]中不存在!`;
...
break;
default:
strMsg = `字段名:[${strFldName}]在表对象:[QxUserGroupPotence]中不存在!(in ${this.constructor.name}.${strThisFuncName})`;
console.error(strMsg);
break;
}
}
 ...
 * (AutoGCLib.EntityLayer4TypeScript:Gen_EN_PropertyNameConst)
 */
 public static readonly con_GroupId = "groupId";    //组Id
 ...
 * (AutoGCLib.EntityLayer4TypeScript:Gen_EN_PropertyNameConst)
 */
 public static readonly con_RelaMid = "relaMid";    //流水号
...
//return propName in new clsQxUserGroupPotenceEN();
const instance = new clsQxUserGroupPotenceEN();
