 ...
 /**
 * 类名:CacheUseStateCRUD(界面:CacheUseStateCRUD,00140047)
 * 表名:CacheUseState(00140136)
 * 版本:2026.04.18.2(服务器:WIN-SRV103-116)
 * 日期:2026/04/18 23:25:53
 * 生成者:
 工程名称:统一平台(0014)
...
import { UserId_Local,CombineCacheUseStateConditionObj4ExportExcel,CombineCacheUseStateConditionObj,divVarSet,viewVarSet,dataColumn,BindTabByList,refCacheUseState_List } from "@/viewsShare/SystemSet/CacheUseStateVueShare";
...
import { CacheUseStateEx_CopyToEx } from "@/ts/L3ForWApiEx/SystemSet/clsCacheUseStateExWApi";
...
try
{
await this.ExtendTdFldFuncMap(arrCacheUseStateExObjLst);
}
catch (e)
{
 ...
 * @param arrDataColumn:用于绑定表的数据列信息
 **/
public async ExtendTdFldFuncMap(arrCacheUseStateExObjLst: Array<clsCacheUseStateENEx>) {
const arrFldName = clsCacheUseStateEN._AttributeName;
const tdFieldNames = refCacheUseState_List.value?.tdFieldNames;
for (const strFldName of tdFieldNames) {
if (IsNullOrEmpty(strFldName) == true) continue;
if (arrFldName.indexOf(strFldName) > -1) continue;
for (const objInFor of arrCacheUseStateExObjLst) {
await CacheUseState_FuncMapByFldName(strFldName, objInFor);
}
}
}
