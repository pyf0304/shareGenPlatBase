...
if (clsQxRoleRightRelationEN._AttributeName.indexOf(strOutFldName) == -1)
{
const strMsg = Format("输出字段名:[{0}]不正确,不在输出字段范围之内!({1})",
strOutFldName, clsQxRoleRightRelationEN._AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (IsNullOrEmpty(clsQxRoleRightRelationEN._WhereFormat) == false)
{
strWhereCond = clsQxRoleRightRelationEN._WhereFormat;
}
if (IsNullOrEmpty(clsQxRoleRightRelationEN._CacheAddiCondition) == false)
{
strWhereCond += Format(" and {0}", clsQxRoleRightRelationEN._CacheAddiCondition);
}
if (strKey == "")
{
console.error("关键字为空!不正确");
...
switch (clsQxRoleRightRelationEN._CacheModeId)
{
