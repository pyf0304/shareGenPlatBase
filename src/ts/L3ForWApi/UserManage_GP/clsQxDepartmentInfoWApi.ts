...
import { qxDepartmentInfoCache,isFuncMapCache } from "@/viewsShare/UserManage_GP/QxDepartmentInfoVueShare";
 ...
 * @returns 对象列表
*/
export  async function QxDepartmentInfo_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsQxDepartmentInfoENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
 const objSortInfo = GetSortExpressInfo(objPagerPara);
const isFuncMapKey = `${ objSortInfo.SortFld}`;
const arrQxDepartmentInfoObjLst = await QxDepartmentInfo_GetObjLstCache();
//从缓存中获取对象，如果缓存中不存在就扩展复制
const arrNewObj = new Array<clsQxDepartmentInfoENEx>();
const arrQxDepartmentInfoExObjLst = arrQxDepartmentInfoObjLst.map((obj) => {
const cacheKey = `${ obj.departmentId}`;
if (qxDepartmentInfoCache[cacheKey])
{
const oldObj = qxDepartmentInfoCache[cacheKey];
return oldObj;
}
else
{
const newObj = QxDepartmentInfo_CopyToEx(obj);
arrNewObj.push(newObj);
qxDepartmentInfoCache[cacheKey] = newObj;
return newObj;
}
});
for (const newObj of arrNewObj) {
for (const strFldName of Object.keys(isFuncMapCache)) {
await QxDepartmentInfo_FuncMapByFldName(strFldName, newObj);
}
}
...
{
for (const newObj of arrQxDepartmentInfoExObjLst) {
await QxDepartmentInfo_FuncMapByFldName(objSortInfo.SortFld, newObj);
const cacheKey = `${ newObj.departmentId}`;
qxDepartmentInfoCache[cacheKey] = newObj;
}
...
const intEnd = intStart + objPagerPara.pageSize;
if (objPagerPara.orderBy != null && objPagerPara.orderBy.length>0) {
arrQxDepartmentInfoSel = arrQxDepartmentInfoSel.sort(QxDepartmentInfo_SortFunByExKey(objSortInfo.SortFld, objSortInfo.SortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrQxDepartmentInfoSel = arrQxDepartmentInfoSel.sort(objPagerPara.sortFun);
 ...
 **/
export  function QxDepartmentInfo_CopyToEx(objQxDepartmentInfoENS:clsQxDepartmentInfoEN ): clsQxDepartmentInfoENEx
{
const strThisFuncName  = QxDepartmentInfo_CopyToEx.name;
...
{
const strMsg = Format("(errid:Watl001294)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxDepartmentInfo_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objQxDepartmentInfoENT;
}
...
*/
export  function QxDepartmentInfo_FuncMapByFldName(strFldName: string, objQxDepartmentInfoEx: clsQxDepartmentInfoENEx)
{
const strThisFuncName = QxDepartmentInfo_FuncMapByFldName.name;
 ...
 * @returns 返回两个对象比较的结果
*/
export  function QxDepartmentInfo_SortFunByExKey(strKey:string, AscOrDesc: string)
{
