/**
 * 类名:LoginLogCRUD(界面:LoginLogCRUD,01120242)
 * 表名:LoginLog(01120357)
 * 版本:2025.02.27.1(服务器:WIN-SRV103-116)
 * 日期:2025/02/27 15:28:50
 * 生成者:
 工程名称:问卷调查(0112)
...
import {
  ...
  LoginLogEx_GetObjExLstByPagerAsync,
} from '@/ts/L3ForWApiEx/LogManage/clsLoginLogExWApi';
import {
...
export abstract class LoginLogCRUD implements clsOperateList {
  ...
  public async ExportExcel_LoginLog() {
    ...
    if (viewVarSet.sortLoginLogBy == null) {
      const strMsg = Format(
  ...
  public async btnExportExcel_Click() {
    await this.ExportExcel_LoginLog();
  }
  ...
  public async BindTab_LoginLog(
    ...
    if (refLoginLog_List.value != null) {
      ...
      if (divDataLst == null) {
        alert('在BindTab_LoginLog函数中，divDataLst不存在!');
        return;
      }
      await BindTab(
        divDataLst,
  ...
  public async IndexPage(intPageIndex: number) {
    ...
    await this.BindGv_LoginLog(this.listPara.listDiv);
  }
   ...
   **/
  public async BindGv_LoginLog(divList: HTMLDivElement) {
    const strThisFuncName = this.BindGv_LoginLog.name;
    if (viewVarSet.sortLoginLogBy == null) {
      const strMsg = Format(
        '在显示列表时,排序字段(sortLoginLogBy)为空,请检查!(In BindGv_LoginLog)',
      );
      console.error(strMsg);
      alert(strMsg);
      return;
    ...
    try {
      ...
      if (this.recCount == 0) {
        ...
        //console.trace();
        alert(strMsg);
        BindTabByList(arrLoginLogExObjLst, true);
        return;
      }
      ...
      const objPagerPara: stuPagerPara = {
        ...
        orderBy: viewVarSet.sortLoginLogBy, //如果该字段为空,就使用下面的排序函数
        sortFun: (x, y) => {
          console.log(x, y);
          return 0;
        },
      };
      arrLoginLogExObjLst = await LoginLogEx_GetObjExLstByPagerAsync(objPagerPara);
    } catch (e) {
      const strMsg = `绑定GridView不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
      console.error('Error: ', strMsg);
      //console.trace();
  ...
  public async SortBy(objAnchorElement: any) {
    ...
    viewVarSet.sortLoginLogBy = sortBy;
