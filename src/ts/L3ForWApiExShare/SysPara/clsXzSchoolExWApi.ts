...
export async function XzSchoolEx_GetObjExLstByPagerCache(
  ...
  const objXzSchool_Cond = new clsXzSchoolENEx();
  ObjectAssign(objXzSchool_Cond, obj_Cond);
  ...
  try {
    ...
    for (const strKey of sstrKeys) {
      ...
      switch (strType) {
        case 'string':
          ...
          if (strComparisonOp == '=') {
            arrXzSchool_Sel = arrXzSchool_Sel.filter(
    ...
    if (arrXzSchool_Sel.length == 0) return arrXzSchool_Sel;
