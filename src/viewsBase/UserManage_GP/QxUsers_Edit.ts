 ...
 /**
 * 类名:QxUsers_Edit(界面:QxUsersCRUD,00140033)
 * 表名:QxUsers(00140015)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/13 16:02:34
 * 生成者:
 工程名称:统一平台(0014)
...
import { QxUsers_CheckPropertyNew,QxUsers_IsExistAsync,QxUsers_AddNewRecordAsync,QxUsers_AddNewRecordWithMaxIdAsync,QxUsers_GetObjByUserIdAsync,QxUsers_CheckProperty4Update,QxUsers_UpdateRecordAsync } from "@/ts/L3ForWApi/UserManage_GP/clsQxUsersWApi";
...
{
 refQxUsers_Edit.value.hideDialog();
}
}
...
await refQxUsers_Edit.value.showDialog(this);
}
 ...
 **/
public async btnUpdateRecordInTab_Click(strUserId: string) {
...
if (this.iShowList != null) this.iShowList.BindGv(clsQxUsersEN._CurrTabName, returnKeyId);
}
}
else
{
...
if (this.iShowList != null) this.iShowList.BindGv(clsQxUsersEN._CurrTabName, this.keyId);
}
}
break;
case "确认修改":
...
try
{
objQxUsersEN = await refQxUsers_Edit.value.GetEditDataQxUsersObj();
}
catch(e)
{
...
const strInfo = `添加[用户(QxUsers)]记录成功!`;
//显示信息框
...
console.error(strMsg);
        ...
        //3、用提供的关键字初始化一个类对象；
refQxUsers_Edit.value.ShowDataFromQxUsersObj(objQxUsersEN);
}
...
await refQxUsers_Edit.value.ShowDataFromQxUsersObj(objQxUsersEN);
...
const objQxUsersEN = await refQxUsers_Edit.value.GetEditDataQxUsersObj();
...
{
//QxUsers_ReFreshCache();
//QxUsers_DeleteKeyIdCache(this.keyId);
}
return returnBool;
}
catch(e)
...
refQxUsers_Edit.value.strCancelButtonText = value;
}
 /**
 * 获取按钮的标题
 **/
public  get btnSubmitQxUsers():string {
const strValue = refQxUsers_Edit.value.strSubmitButtonText;
return strValue;
}
 /**
...
refQxUsers_Edit.value.strSubmitButtonText = value;
}
}