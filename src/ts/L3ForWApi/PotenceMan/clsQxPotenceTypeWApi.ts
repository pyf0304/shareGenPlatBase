...
if (clsQxPotenceTypeEN._AttributeName.indexOf(strOutFldName) == -1)
{
const strMsg = Format("输出字段名:[{0}]不正确,不在输出字段范围之内!({1})",
strOutFldName, clsQxPotenceTypeEN._AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (IsNullOrEmpty(clsQxPotenceTypeEN._WhereFormat) == false)
{
strWhereCond = clsQxPotenceTypeEN._WhereFormat;
}
if (IsNullOrEmpty(clsQxPotenceTypeEN._CacheAddiCondition) == false)
{
strWhereCond += Format(" and {0}", clsQxPotenceTypeEN._CacheAddiCondition);
}
if (strKey == "")
{
console.error("关键字为空!不正确");
...
switch (clsQxPotenceTypeEN._CacheModeId)
{
 ...
 * @returns 对象列表
*/
export  async function QxPotenceType_GetObjExLstByPagerCache(objPagerPara: stuPagerPara ):Promise<Array<clsQxPotenceTypeENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
...
clsQxPotenceTypeEN._AttributeName.indexOf(objSortInfo.SortFld) == -1 &&
...
const intEnd = intStart + objPagerPara.pageSize;
if (objPagerPara.orderBy != null && objPagerPara.orderBy.length>0) {
arrQxPotenceTypeSel = arrQxPotenceTypeSel.sort(QxPotenceType_SortFunByExKey(objSortInfo.SortFld, objSortInfo.SortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrQxPotenceTypeSel = arrQxPotenceTypeSel.sort(objPagerPara.sortFun);
 ...
 **/
export  function QxPotenceType_CopyToEx(objQxPotenceTypeENS:clsQxPotenceTypeEN ): clsQxPotenceTypeENEx
{
...
{
const strMsg = Format("(errid:Watl001294)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxPotenceType_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objQxPotenceTypeENT;
}
...
*/
export  function QxPotenceType_FuncMapByFldName(strFldName: string, objQxPotenceTypeEx: clsQxPotenceTypeENEx)
{
...
{
const strMsg = Format("(errid:Watl001372)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPotenceType_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
{
const strMsg = Format("(errid:Watl001494)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPotenceType_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
{
const strMsg = Format("(errid:Watl001326)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPotenceType_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
