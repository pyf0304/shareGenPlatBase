...
import {
  ...
  vStudentInfo_Sim_GetObjLstByPagerAsync,
  vStudentInfo_Sim_SortFunByKey,
  vStudentInfo_Sim_FilterFunByKey,
  vStudentInfo_Sim_GetFirstObjAsync,
 ...
 * @returns 对象列表
 */
export async function vStudentInfo_SimEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvStudentInfo_SimENEx>> {
  const strThisFuncName = 'GetObjExLstByPagerAsync';
  const arrvStudentInfo_SimObjLst = await vStudentInfo_Sim_GetObjLstByPagerAsync(objPagerPara);
  const arrvStudentInfo_SimExObjLst = arrvStudentInfo_SimObjLst.map(vStudentInfo_SimEx_CopyToEx);
  if (arrvStudentInfo_SimExObjLst.length == 0) return arrvStudentInfo_SimExObjLst;
  let arrvStudentInfo_SimSel: Array<clsvStudentInfo_SimENEx> = arrvStudentInfo_SimExObjLst;
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvStudentInfo_SimSel = arrvStudentInfo_SimSel.sort(
        vStudentInfo_SimEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
  ...
  return new Array<clsvStudentInfo_SimENEx>();
}
...
export async function vStudentInfo_SimEx_GetObjByStuId(
  strStuId: string,
): Promise<clsvStudentInfo_SimEN | null> {
  const strThisFuncName = vStudentInfo_SimEx_GetObjByStuId.name;
  // console.log(objvStudentInfo_SimEx);
  // let strMsg = '';
  const strWhere = `${clsvStudentInfo_SimEN.con_StuId} = '${strStuId}'`;
  //如果是本表中字段,不需要映射
