<template>
  ...
  <el-dialog v-model="dialogVisible" :width="dialogWidth" :show-close="false">
    ...
    <template #footer>
      ...
      <el-button id="btnSubmitQxPrjUserRelation" type="primary" @click="btnSubmit_Click">{{
        strSubmitButtonText
      }}</el-button>
    </template>
  </el-dialog>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
       ...
       * @param pobjQxPrjUserRelationEN">数据传输的目的类对象</param>
       **/
      async function GetEditDataQxPrjUserRelationObj() {
        const pobjQxPrjUserRelationEN = new clsQxPrjUserRelationEN();
        pobjQxPrjUserRelationEN.SetQxPrjId(qxPrjId.value); // 项目Id
        pobjQxPrjUserRelationEN.SetUserId(userId.value); // 用户ID
        pobjQxPrjUserRelationEN.SetUpdDate(clsDateTime.getTodayDateTimeStr(1)); // 修改日期
        pobjQxPrjUserRelationEN.SetMemo(memo.value); // 备注
      ...
      async function ShowDataFromQxPrjUserRelationObj(
        pobjQxPrjUserRelationEN: clsQxPrjUserRelationEN,
      ) {
        qxPrjId.value = pobjQxPrjUserRelationEN.qxPrjId; // 项目Id
        userId.value = pobjQxPrjUserRelationEN.userId; // 用户ID
        memo.value = pobjQxPrjUserRelationEN.memo; // 备注
      }
      ...
      const showDialog = async (pobjPage_Edit: QxPrjUserRelation_EditEx) => {
      ...
      return {
        ...
        strSubmitButtonText,
        strCancelButtonText,
        GetEditDataQxPrjUserRelationObj,
    ...
    methods: {
       ...
       **/
      btnQxPrjUserRelation_Edit_Click(strCommandName: string, strKeyId: string) {
        QxPrjUserRelation_EditEx.btnEdit_Click(strCommandName, strKeyId);
      },
    },
  });
</script>
