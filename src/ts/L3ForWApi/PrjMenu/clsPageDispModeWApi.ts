...
export async function PageDispMode_UpdateObjInLstCache(objPageDispMode: clsPageDispModeEN) {
  ...
  try {
    ...
    const obj = arrPageDispModeObjLstCache.find(
      (x) => x.pageDispModeName == objPageDispMode.pageDispModeName,
    );
    if (obj != null) {
...
export async function PageDispMode_GetSubObjLstCache(objPageDispModeCond: clsPageDispModeEN) {
  ...
  if (objPageDispModeCond.sfFldComparisonOp == null || objPageDispModeCond.sfFldComparisonOp == '')
    return arrPageDispModeSel;
  const dicFldComparisonOp: { [index: string]: string } = JSON.parse(
    objPageDispModeCond.sfFldComparisonOp,
  );
 ...
 * @returns 获取的相应记录对象列表
 **/
export async function PageDispMode_GetObjLstByPagerAsync(
 ...
 **/
export async function PageDispMode_CheckUniCond4Add(
  objPageDispModeEN: clsPageDispModeEN,
): Promise<boolean> {
 ...
 **/
export async function PageDispMode_CheckUniCond4Update(
  objPageDispModeEN: clsPageDispModeEN,
): Promise<boolean> {
...
export function PageDispMode_CheckPropertyNew(pobjPageDispModeEN: clsPageDispModeEN) {
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000413)字段[备注(memo)]的长度不能超过1000(In 页面显示模式(PageDispMode))!值:${pobjPageDispModeEN.memo}(clsPageDispModeBL:CheckPropertyNew)`,
    );
  }
  //检查字段的数据类型是否正确
  if (
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000414)字段[备注(memo)]的值:[${pobjPageDispModeEN.memo}], 非法,应该为字符型(In 页面显示模式(PageDispMode))!(clsPageDispModeBL:CheckPropertyNew0)`,
    );
  }
...
export function PageDispMode_CheckProperty4Update(pobjPageDispModeEN: clsPageDispModeEN) {
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000416)字段[备注(memo)]的长度不能超过1000(In 页面显示模式(PageDispMode))!值:${pobjPageDispModeEN.memo}(clsPageDispModeBL:CheckProperty4Update)`,
    );
  }
  //检查字段的数据类型是否正确
  if (
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000417)字段[备注(memo)]的值:[${pobjPageDispModeEN.memo}], 非法,应该为字符型(In 页面显示模式(PageDispMode))!(clsPageDispModeBL:CheckProperty4Update)`,
    );
  }
  //检查主键是否为Null或者空!
  //检查外键, 作为外键应该和主键的字段长度是一样的, 若不一样,即非法!
 ...
 */
export function PageDispMode_CopyObjTo(
  objPageDispModeENS: clsPageDispModeEN,
  objPageDispModeENT: clsPageDispModeEN,
): void {
