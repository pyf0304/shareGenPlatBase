...
export const CombineQxUsersConditionObj = async (): Promise<ConditionCollection> => {
  ...
  try {
    ...
    if (idSchool_q.value != '' && idSchool_q.value != '0') {
      strWhereCond += Format(" And {0} = '{1}'", clsQxUsersEN.con_Id_School, idSchool_q.value);
      objQxUsersCond.SetCondFldValue(clsQxUsersEN.con_Id_School, idSchool_q.value, '=');
    }
...
export function QxUsers_DeleteKeyIdCache(strUserId: string): void {
  if (IsNullOrEmpty(strUserId) == false) {
    // 使用 delete 删除特定的键
