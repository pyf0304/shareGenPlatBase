...
import { qxPrjPotenceCache } from '@/viewsShare/PotenceMan/QxPrjPotenceVueShare';
import { isFuncMapCache } from '@/viewsShare/PotenceMan/QxPrjPotenceVueShare';
...
export async function QxPrjPotenceEx_GetObjExLstByPagerCache(
  ...
  const arrQxPrjPotenceExObjLst = arrQxPrjPotenceObjLst.map((obj) => {
    ...
    if (qxPrjPotenceCache[cacheKey]) {
      ...
      const newObj = QxPrjPotenceEx_CopyToEx(obj);
  ...
  for (const newObj of arrNewObj) {
    for (const strFldName of Object.keys(isFuncMapCache)) {
      await QxPrjPotenceEx_FuncMapByFldName(strFldName, newObj);
    }
  }
  ...
  if (
    ...
    for (const newObj of arrQxPrjPotenceExObjLst) {
      await QxPrjPotenceEx_FuncMapByFldName(objSortInfo.SortFld, newObj);
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      arrQxPrjPotenceSel = arrQxPrjPotenceSel.sort(
        QxPrjPotenceEx_SortFunByKey(objSortInfo.SortFld, objSortInfo.SortType),
      );
    } else {
      //如果排序字段名[OrderBy]为空,就调用排序函数
      arrQxPrjPotenceSel = arrQxPrjPotenceSel.sort(objPagerPara.sortFun);
