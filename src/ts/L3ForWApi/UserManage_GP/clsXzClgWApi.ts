 ...
 * @returns 对象列表子集
 */
export async function XzClg_GetSubObjLstCache(
  objXzClgCond: ConditionCollection,
  strIdSchool: string,
) {
...
export async function XzClg_AddNewObjSave(objXzClgEN: clsXzClgEN): Promise<AddRecordResult> {
  ...
  try {
    ...
    if (returnBool == true) {
      XzClg_ReFreshCache(objXzClgEN.id_School);
    } else {
      const strInfo = `添加[XzClg(XzClg)]记录不成功!`;
      //显示信息框
      throw strInfo;
    }
 ...
 * @returns 对象列表子集
 */
export async function XzClg_IsExistRecordCache(
  objXzClgCond: ConditionCollection,
  strIdSchool: string,
) {
 ...
 * @returns 对象列表记录数
 */
export async function XzClg_GetRecCountByCondCache(
  objXzClgCond: ConditionCollection,
  strIdSchool: string,
) {
 ...
 * @param strIdSchool:
*/
export async function XzClg_GetArrXzClgByIdSchool(strIdSchool: string) {
  if (IsNullOrEmpty(strIdSchool) == true) {
    const strMsg = Format(
      '参数:[strIdSchool]不能为空！(In clsXzClgWApi.BindDdl_IdXzCollegeByIdSchoolInDiv)',
    );
...
export function XzClg_CheckPropertyNew(pobjXzClgEN: clsXzClgEN) {
  ...
  if (IsNullOrEmpty(pobjXzClgEN.userTypeId) == false && GetStrLen(pobjXzClgEN.userTypeId) > 2) {
    throw new Error(
      `(errid:Watl000413)字段[用户类型Id(userTypeId)]的长度不能超过2(In XzClg(XzClg))!值:${pobjXzClgEN.userTypeId}(clsXzClgBL:CheckPropertyNew)`,
    );
  }
  if (
  ...
  if (IsNullOrEmpty(pobjXzClgEN.id_School) == false && GetStrLen(pobjXzClgEN.id_School) > 4) {
    throw new Error(
      `(errid:Watl000413)字段[学校流水号(idSchool)]的长度不能超过4(In XzClg(XzClg))!值:${pobjXzClgEN.id_School}(clsXzClgBL:CheckPropertyNew)`,
    );
  }
  ...
  if (
    IsNullOrEmpty(pobjXzClgEN.userTypeId) == false &&
    undefined !== pobjXzClgEN.userTypeId &&
    tzDataType.isString(pobjXzClgEN.userTypeId) === false
  ) {
    throw new Error(
      `(errid:Watl000414)字段[用户类型Id(userTypeId)]的值:[${pobjXzClgEN.userTypeId}], 非法,应该为字符型(In XzClg(XzClg))!(clsXzClgBL:CheckPropertyNew0)`,
    );
  }
  if (
    IsNullOrEmpty(pobjXzClgEN.collegeIdInGP) == false &&
  ...
  if (
    IsNullOrEmpty(pobjXzClgEN.id_School) == false &&
    undefined !== pobjXzClgEN.id_School &&
    tzDataType.isString(pobjXzClgEN.id_School) === false
  ) {
    throw new Error(
      `(errid:Watl000414)字段[学校流水号(idSchool)]的值:[${pobjXzClgEN.id_School}], 非法,应该为字符型(In XzClg(XzClg))!(clsXzClgBL:CheckPropertyNew0)`,
    );
  }
  if (
...
export function XzClg_CheckProperty4Update(pobjXzClgEN: clsXzClgEN) {
  ...
  if (IsNullOrEmpty(pobjXzClgEN.userTypeId) == false && GetStrLen(pobjXzClgEN.userTypeId) > 2) {
    throw new Error(
      `(errid:Watl000416)字段[用户类型Id(userTypeId)]的长度不能超过2(In XzClg(XzClg))!值:${pobjXzClgEN.userTypeId}(clsXzClgBL:CheckProperty4Update)`,
    );
  }
  if (
    IsNullOrEmpty(pobjXzClgEN.collegeIdInGP) == false &&
  ...
  if (IsNullOrEmpty(pobjXzClgEN.id_School) == false && GetStrLen(pobjXzClgEN.id_School) > 4) {
    throw new Error(
      `(errid:Watl000416)字段[学校流水号(idSchool)]的长度不能超过4(In XzClg(XzClg))!值:${pobjXzClgEN.id_School}(clsXzClgBL:CheckProperty4Update)`,
    );
  }
  if (IsNullOrEmpty(pobjXzClgEN.idUni) == false && GetStrLen(pobjXzClgEN.idUni) > 4) {
    throw new Error(
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000417)字段[用户类型Id(userTypeId)]的值:[${pobjXzClgEN.userTypeId}], 非法,应该为字符型(In XzClg(XzClg))!(clsXzClgBL:CheckProperty4Update)`,
    );
  }
  if (
    IsNullOrEmpty(pobjXzClgEN.collegeIdInGP) == false &&
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000417)字段[学校流水号(idSchool)]的值:[${pobjXzClgEN.id_School}], 非法,应该为字符型(In XzClg(XzClg))!(clsXzClgBL:CheckProperty4Update)`,
    );
  }
  if (
    IsNullOrEmpty(pobjXzClgEN.idUni) == false &&
...
export function XzClg_GetCombineCondition(objXzClgCond: clsXzClgEN): string {
  ...
  if (
    ...
    const strComparisonOpIdXzCollege: string =
      objXzClgCond.dicFldComparisonOp[clsXzClgEN.con_Id_XzCollege];
    strWhereCond += Format(
      " And {0} {2} '{1}'",
      clsXzClgEN.con_Id_XzCollege,
  ...
  if (
    Object.prototype.hasOwnProperty.call(
      objXzClgCond.dicFldComparisonOp,
      clsXzClgEN.con_UserTypeId,
    ) == true
  ) {
    const strComparisonOpUserTypeId: string =
      objXzClgCond.dicFldComparisonOp[clsXzClgEN.con_UserTypeId];
    strWhereCond += Format(
      " And {0} {2} '{1}'",
      clsXzClgEN.con_UserTypeId,
      objXzClgCond.userTypeId,
