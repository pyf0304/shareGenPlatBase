 ...
 * @returns 对象
 **/
export  async function vQxUsersSim_GetObjByUserIdAsync(strUserId: string): Promise<clsvQxUsersSimEN|null>  
{
...
{
const strInfo = Format("网络错误!访问地址:{0}不成功!(in {1}.{2})", strUrl, vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strInfo);
throw(strInfo);
}
else if (error.statusText == "Not Found")
{
const strInfo = Format("网络错误!访问地址:{0}可能不存在!(in {1}.{2})", strUrl, vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strInfo);
throw(strInfo);
}
else
...
if (IsNullOrEmpty(strUserId) == true)
{
  const strMsg = Format("参数:[strUserId]不能为空!(In clsvQxUsersSimWApi.GetObjByUserIdlocalStorage)");
console.error(strMsg);
 throw (strMsg);
}
...
{
const strMsg = Format("错误:[{0}]. \n根据关键字:[{1}]获取相应的对象不成功!(in {2}.{3})", e, strUserId, vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
return;
}
...
try
{
const arrvQxUsersSimSel = arrvQxUsersSimObjLstCache.filter(x => 
 x.userId == strUserId );
 ...
 * @returns 返回一个输出字段值
*/
export  async function vQxUsersSim_func(strInFldName:string , strOutFldName:string , strInValue:string )
{
...
if (clsvQxUsersSimEN._AttributeName.indexOf(strOutFldName) == -1)
{
const strMsg = Format("输出字段名:[{0}]不正确,不在输出字段范围之内!({1})",
strOutFldName, clsvQxUsersSimEN._AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 返回一个关键字值列表
*/
export  async function vQxUsersSim_funcKey(strInFldName:string, strInValue: any, strComparisonOp:string): Promise<Array<string>> 
{
...
arrvQxUsersSimSel = arrvQxUsersSimSel.filter(x => x.GetFldValue(strInFldName).toString() == strInValue.toString());
break;
case enumComparisonOp.Like_03:
arrvQxUsersSimSel = arrvQxUsersSimSel.filter(x => x.GetFldValue(strInFldName).toString().indexOf(strInValue.toString()) != -1);
break;
case enumComparisonOp.In_04:
arrValues = strInValue.split(',');
arrvQxUsersSimSel = arrvQxUsersSimSel.filter(x => arrValues.indexOf(x.GetFldValue(strInFldName).toString()) != -1);
break;
}
break;
case "boolean":
...
{
arrvQxUsersSimSel = arrvQxUsersSimSel.filter(x => x.GetFldValue(strInFldName) == strInValue);
}
break;
case "number":
 ...
 **/
export  async function vQxUsersSim_GetFldValueAsync(strFldName: string, strWhereCond: string): Promise<Array<string>>  
{
 ...
 * @returns 第一条记录对象
 **/
export  async function vQxUsersSim_GetFirstObjAsync(strWhereCond: string): Promise<clsvQxUsersSimEN|null>  
{
...
if (IsNullOrEmpty(clsvQxUsersSimEN._WhereFormat) == false)
{
strWhereCond = clsvQxUsersSimEN._WhereFormat;
}
if (IsNullOrEmpty(clsvQxUsersSimEN._CacheAddiCondition) == false)
{
strWhereCond += Format(" and {0}", clsvQxUsersSimEN._CacheAddiCondition);
}
if (strKey == "")
{
console.error("关键字为空!不正确");
...
const strInfo = Format("[ClientCache]Key:[{0}]的缓存已经建立,对象列表数：{1}!", strKey, arrvQxUsersSimExObjLst.length);
console.log(strInfo);
return arrvQxUsersSimExObjLst;
}
catch (e)
{
const strMsg = Format("从本地缓存中获取所有对象列表出错. \n服务器错误：{0}.(in {1}.{2})", e, vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strMsg);
throw (strMsg);
}
}
...
const strInfo = Format("[localStorage]Key:[{0}]的缓存已经建立,对象列表数：{1}!", strKey, arrvQxUsersSimExObjLst.length);
console.log(strInfo);
return arrvQxUsersSimExObjLst;
}
catch (e)
 ...
 * @returns 获取的相应对象列表
 **/
export  async function vQxUsersSim_GetObjLstAsync(strWhereCond: string): Promise<Array<clsvQxUsersSimEN>>  
{
...
{
const strNullInfo = Format("获取数据为null, 请注意!(in {0}.{1})", vQxUsersSim_ConstructorName, strThisFuncName);
...
const strInfo = Format("[sessionStorage]Key:[{0}]的缓存已经建立,对象列表数：{1}!", strKey, arrvQxUsersSimExObjLst.length);
console.log(strInfo);
return arrvQxUsersSimExObjLst;
}
catch (e)
...
switch (clsvQxUsersSimEN._CacheModeId)
{
...
if (strComparisonOp == "=") {
arrvQxUsersSimSel = arrvQxUsersSimSel.filter(x => x.GetFldValue(strKey).toString() == strValue.toString());
}
else if (strComparisonOp == "like") {
arrvQxUsersSimSel = arrvQxUsersSimSel.filter(x => x.GetFldValue(strKey).toString().indexOf(strValue.toString()) != -1);
}
else if (strComparisonOp == "length greater") {
arrvQxUsersSimSel = arrvQxUsersSimSel.filter(x => x.GetFldValue(strKey).toString().length > Number(strValue.toString()));
}
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]缓存对象列表中获取子集对象不成功!(in {2}.{3})", e, JSON.stringify( objvQxUsersSimCond), vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
 ...
 * @returns 对象列表
 **/
export  async function vQxUsersSim_GetObjLstByUserIdLstAsync(arrUserId: Array<string>): Promise<Array<clsvQxUsersSimEN>>  
{
...
{
const arrvQxUsersSimObjLstCache = await vQxUsersSim_GetObjLstCache();
const arrvQxUsersSimSel = arrvQxUsersSimObjLstCache.filter(x => arrUserIdLst.indexOf(x.userId)>-1);
return arrvQxUsersSimSel;
}
catch (e)
{
const strMsg = Format("错误:[{0}]. \n根据关键字:[{1}]获取对象列表不成功!(in {2}.{3})", e, arrUserIdLst.join(","), vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
}
 ...
 * @returns 获取的相应对象列表
 **/
export  async function vQxUsersSim_GetTopObjLstAsync(objTopPara: stuTopPara): Promise<Array<clsvQxUsersSimEN>>  
{
 ...
 * @returns 获取的相应记录对象列表
 **/
export  async function vQxUsersSim_GetObjLstByRangeAsync(objRangePara: stuRangePara): Promise<Array<clsvQxUsersSimEN>>  
{
...
let arrvQxUsersSimSel = arrvQxUsersSimObjLstCache;
if (objvQxUsersSimCond.GetConditions().length == 0) return arrvQxUsersSimSel.length>0?true:false;
try {
for (const objCondition of objvQxUsersSimCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
...
const strMsg = Format("根据条件:[{0}]判断是否存在不成功!(in {1}.{2})", JSON.stringify( objvQxUsersSimCond), vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
return false;
...
{
const strMsg = Format("根据关键字:[{0}]判断是否存在不成功!(in {1}.{2})", strUserId, vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
return false;
...
arrvQxUsersSimSel = arrvQxUsersSimSel.filter(x => arrValues.indexOf(x.GetFldValue(strKey).toString()) != -1);
}
break;
case "boolean":
if (strValue == null) continue;
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]从缓存对象列表中获取记录数不成功!(in {2}.{3})", e, JSON.stringify( objvQxUsersSimCond), vQxUsersSim_ConstructorName, strThisFuncName);
console.error(strMsg);
throw new Error(strMsg);
}
return 0;
...
*/
export  async function vQxUsersSim_BindDdl_UserIdInDivCache(objDiv: HTMLDivElement, strDdlName: string )
{
...
BindDdl_ObjLstInDivObj_V(objDiv, strDdlName, arrObjLstSel, clsvQxUsersSimEN.con_UserId, clsvQxUsersSimEN.con_UserName, "v用户Sim...");
}
 ...
 * @returns 返回一个生成的对象列表
*/
export  function vQxUsersSim_GetObjLstByJSONObjLst (arrvQxUsersSimObjLstS: Array<clsvQxUsersSimEN>): Array<clsvQxUsersSimEN>
{
...
if (Object.prototype.hasOwnProperty.call(objvQxUsersSimCond.dicFldComparisonOp, clsvQxUsersSimEN.con_UserId) == true)
{
const strComparisonOpUserId:string = objvQxUsersSimCond.dicFldComparisonOp[clsvQxUsersSimEN.con_UserId];
strWhereCond += Format(" And {0} {2} '{1}'", clsvQxUsersSimEN.con_UserId, objvQxUsersSimCond.userId, strComparisonOpUserId);
}
if (Object.prototype.hasOwnProperty.call(objvQxUsersSimCond.dicFldComparisonOp, clsvQxUsersSimEN.con_UserName) == true)
{
const strComparisonOpUserName:string = objvQxUsersSimCond.dicFldComparisonOp[clsvQxUsersSimEN.con_UserName];
strWhereCond += Format(" And {0} {2} '{1}'", clsvQxUsersSimEN.con_UserName, objvQxUsersSimCond.userName, strComparisonOpUserName);
}
if (Object.prototype.hasOwnProperty.call(objvQxUsersSimCond.dicFldComparisonOp, clsvQxUsersSimEN.con_Id_XzCollege) == true)
{
const strComparisonOpId_XzCollege:string = objvQxUsersSimCond.dicFldComparisonOp[clsvQxUsersSimEN.con_Id_XzCollege];
strWhereCond += Format(" And {0} {2} '{1}'", clsvQxUsersSimEN.con_Id_XzCollege, objvQxUsersSimCond.id_XzCollege, strComparisonOpId_XzCollege);
}
if (Object.prototype.hasOwnProperty.call(objvQxUsersSimCond.dicFldComparisonOp, clsvQxUsersSimEN.con_DepartmentId) == true)
{
const strComparisonOpDepartmentId:string = objvQxUsersSimCond.dicFldComparisonOp[clsvQxUsersSimEN.con_DepartmentId];
strWhereCond += Format(" And {0} {2} '{1}'", clsvQxUsersSimEN.con_DepartmentId, objvQxUsersSimCond.departmentId, strComparisonOpDepartmentId);
}
 return strWhereCond;
}
...
*/
export  function vQxUsersSim_GetObjFromJsonObj(objvQxUsersSimENS: clsvQxUsersSimEN): clsvQxUsersSimEN 
{
