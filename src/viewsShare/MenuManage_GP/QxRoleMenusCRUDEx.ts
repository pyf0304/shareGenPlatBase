...
import {
  QxRoleMenusEx_Delete,
  QxRoleMenusEx_GetMenuIdLstByCmPrjId,
 ...
 **/
export class QxRoleMenusCRUDEx extends QxPrjMenusCRUD implements IShowList {
  ...
  public static btn_Click(strCommandName: string, strKeyId: any) {
    let objPage: QxRoleMenusCRUDEx;
    const objData = strKeyId;
    let objPageEdit;
    if (QxPrjMenusCRUD.objPageCRUD == null) {
      QxPrjMenusCRUD.objPageCRUD = new QxRoleMenusCRUDEx();
      objPage = <QxRoleMenusCRUDEx>QxPrjMenusCRUD.objPageCRUD;
    } else {
      objPage = <QxRoleMenusCRUDEx>QxPrjMenusCRUD.objPageCRUD;
    }
    let strMsg = '';
    ...
    switch (strCommandName) {
      case 'DelRole':
        objPage.btnDelRole_Click(objData.roleId, objData.menuSetId, objData.menuId);
        break;
  ...
  public async PageLoadCache() {
    ...
    try {
      ...
      QxRoleMenusCRUDEx.SetDefaultValue();

      // 为功能区绑定下拉框
      await this.BindDdl4FeatureRegion();
      qryVarSet.inUse_q = 'true';
  ...
  public static SetDefaultValue() {
    ...
    QxRoleMenusCRUDEx.vuebtn_Click('setMenuSetId', strMenuSetId);
  }
  /** 函数功能:为查询区绑定下拉框
        ...
        if (aDel != null) {
          ...
          (function (objData: any) {
            aDel.onclick = function () {
              QxRoleMenusCRUDEx.vuebtn_Click('DelRole', objData);
            };
          })(objData);
        }
      }
  ...
  public async btnSetInUse_Click() {
    ...
    try {
      ...
      const strInUse = GetSelectValueInDivObj(divVarSet.refDivFunction, 'ddlInUse_SetFldValue'); //.prop('checked');
  ...
  public async btnSetRoleId_Click() {
    ...
    try {
      ...
      if (strMenuSetId == '' || strMenuSetId == '0') {
      ...
      menuSetId_f.value = strMenuSetId;
    ...
    try {
      ...
      for (const strMenuId of arrMenuId) {
        ...
        try {
          ...
          const strTemp = await qxRoleMenusStore.getRefreshMenuId(strMenuId);
          console.log(strTemp);
          // qxRoleMenusStore.delMenuIdsByRoleId(strRoleId);
        } catch (e) {
          const strMsg = Format(
            '设置记录不成功,{0}.(in {1}.{2})',
            e,
  ...
  public get editTabName(): string {
    return QxRoleMenusCRUDEx.GetPropValue('editTabName');
  }

  /** 删除记录
   * (AutoGCLib.WA_ViewScriptCS_TS4TypeScript:Gen_WApi_Ts_btnDelRecord_Click)
   **/
  public async btnDelRole_Click(strRoleId: string, strMenuSetId: string, strMenuId: string) {
    const strThisFuncName = this.btnDelRole_Click.name;
    try {
      const qxRoleMenusStore = useQxRoleMenusStore();
      await QxRoleMenusEx_Delete(strRoleId, strMenuSetId, strMenuId);
      qxRoleMenusStore.delMenuId(strMenuId, clsSysPara4WebApi.currSelPrjId);
      const strTemp = await qxRoleMenusStore.getRefreshMenuId(strMenuId);
      console.log(strTemp);
      await this.BindGv_QxPrjMenus4Func(divVarSet.refDivList);
  ...
  public get menuSetId(): string {
    return QxRoleMenusCRUDEx.GetPropValue('menuSetId');
  }
  /** 根据条件获取相应的对象列表
  ...
  public async SortColumn(sortColumnKey: string, sortDirection: string) {
    switch (sortColumnKey) {
      case 'orderNum|Ex':
        ...
        break;
      case 'roleName|Ex':
        viewVarSet.sortQxPrjMenusBy = `QxRoles|roleName ${sortDirection}|QxRoleMenus.RoleId = QxRoles.RoleId`;
        break;
      case 'menuName|Ex':
        viewVarSet.sortQxPrjMenusBy = Format('{0} {1}', sortColumnKey, sortDirection);
        break;
      ...
      case 'pageDispModeName|Ex':
        viewVarSet.sortQxPrjMenusBy = `vQxPrjMenusSim|pageDispModeName ${sortDirection}|QxRoleMenus.MenuId = vQxPrjMenusSim.MenuId,vQxPrjMenusSim.PageDispModeId = QxPageDispMode.PageDispModeId`;
        break;
      case 'inUse|Ex':
        viewVarSet.sortQxPrjMenusBy = Format('{0} {1}', sortColumnKey, sortDirection);
        break;
