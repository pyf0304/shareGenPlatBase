<template>
  <div id="divLayout" ref="refDivLayout" class="div_layout">
    ...
    <div id="divList" ref="refDivList" class="div_List">
      <QxUserIdentity_ListCom
        ref="refQxUserIdentity_List"
        :items="dataListQxUserIdentity"
...
<script lang="ts">
  ...
  import QxUserIdentity_EditEx from '@/viewsShare/UserManage_GP/QxUserIdentity_EditEx';
  import QxUserIdentity_DetailEx from '@/viewsShare/UserManage_GP/QxUserIdentity_DetailEx';
  import QxUserIdentityCRUDEx from '@/viewsShare/UserManage_GP/QxUserIdentityCRUDEx';
  import QxUserIdentity_EditCom from '@/viewsShare/UserManage_GP/QxUserIdentity_Edit.vue';
  import QxUserIdentity_DetailCom from '@/viewsShare/UserManage_GP/QxUserIdentity_Detail.vue';
  import QxUserIdentity_ListCom from '@/viewsShare/UserManage_GP/QxUserIdentity_List.vue';
  ...
  export default defineComponent({
    ...
    setup() {
      const objPage = ref<QxUserIdentityCRUDEx>();
      const objPage_Edit = ref<QxUserIdentity_EditEx>();
      const objPage_Detail = ref<QxUserIdentity_DetailEx>();
      const opType = ref('');
      const thisConstructorName = 'QxUserIdentityCRUD';
...
*/
      const btnDetail_Click = async () => {
        ...
        objPage_Detail.value = new QxUserIdentity_DetailEx(objPage.value);
        if (objPage_Detail.value == null) {
          alert('编辑页面初始化不成功,请联系管理员!');
          return;
        }
        const strIdentityId = GetFirstCheckedKeyIdInDivObj(divVarSet.refDivList);
        opType.value = 'Detail';
        const bolIsSuccess = await objPage_Detail.value.ShowDialog_QxUserIdentity('Detail');
        if (bolIsSuccess == false) return;
        // 为编辑区绑定下拉框
        //const conBindDdl = await this.BindDdl4DetailRegion();
        objPage_Detail.value.DetailRecord(strIdentityId);
      };
      ...
      const btnQuery_Click = async () => {
        ...
        await objPage.value.BindGv_QxUserIdentityCache(refDivList.value);
      };
      ...
      const btnCreate_Click = async () => {
        ...
        objPage_Edit.value = new QxUserIdentity_EditEx('QxUserIdentity_EditEx', objPage.value);
        ...
        try {
          opType.value = 'Add';
          const bolIsSuccess = await objPage_Edit.value.ShowDialog_QxUserIdentity(opType.value);
          if (bolIsSuccess == false) return;
          if (['02', '03', '06'].indexOf(clsQxUserIdentityEN._PrimaryTypeId) > -1) {
      ...
      const btnUpdate_Click = async () => {
        ...
        try {
          ...
          const bolIsSuccess = await objPage_Edit.value.ShowDialog_QxUserIdentity(opType.value);
          if (bolIsSuccess == false) return;
          const update = await objPage_Edit.value.UpdateRecord(strIdentityId);
          if (update == false) {
      ...
      const btnDelete_Click = async () => {
        ...
        try {
          ...
          await objPage.value.BindGv_QxUserIdentityCache(divVarSet.refDivList);
        } catch (e) {
      ...
      const btnExportExcel_Click = async () => {
        ...
        await objPage.value.ExportExcel_QxUserIdentityCache();
      };
      ...
      onMounted(() => {
        ...
        QxUserIdentityCRUDEx.vuebtn_Click = btn_Click;
        QxUserIdentityCRUDEx.GetPropValue = GetPropValue;
        objPage.value = new QxUserIdentityCRUDEx();
      ...
      function btn_Click(strCommandName: string, strKeyId: string) {
        ...
        QxUserIdentityCRUDEx.btn_Click(strCommandName, strKeyId);
      }
      return {
        showErrorMessage,
        dataListQxUserIdentity,
    ...
    methods: {
      ...
      async EditTabRelaInfo(data: any) {
        ...
        router.push({ name: 'editQxUserIdentity', params: { courseId: data.courseId } });
      },
