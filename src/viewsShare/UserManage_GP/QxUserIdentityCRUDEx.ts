/**
 * 类名:QxUserIdentityCRUDEx(界面:QxUserIdentityCRUD,00140039)
 * 表名:QxUserIdentity(00140042)
 * 版本:2024.11.13.2(服务器:WIN-SRV103-116)
 * 日期:2024/11/16 11:34:02
 * 生成者:
 工程名称:统一平台(0014)
...
export default class QxUserIdentityCRUDEx extends QxUserIdentityCRUD implements IShowList {
  ...
  public async InitVarSet(): Promise<void> {
    console.log('InitVarSet in QxUserIdentityCRUDEx');
  }
  /**
   * 函数功能:初始化界面控件值，放在绑定下拉框之后
   **/
  public async InitCtlVar(): Promise<void> {
    console.log('InitCtlVar in QxUserIdentityCRUDEx');
  }
  ...
  public static btn_Click(strCommandName: string, strKeyId: string) {
    ...
    if (QxUserIdentityCRUD.objPageCRUD == null) {
      ...
      objPage = <QxUserIdentityCRUDEx>QxUserIdentityCRUDEx.objPageCRUD;
    } else {
      objPage = <QxUserIdentityCRUDEx>QxUserIdentityCRUDEx.objPageCRUD;
    }
    let strMsg = '';
    ...
    switch (strCommandName) {
      ...
      case 'Create': //添加记录
        objPageEdit = new QxUserIdentity_EditEx('QxUserIdentity_EditEx', objPage); //初始化编辑类,设置当前类为编辑类的父类，编辑返回的类
      ...
      case 'Delete': //删除记录
        ...
        break;
      default:
        strMsg = `命令:${strCommandName}在函数(QxUserIdentityCRUDEx.btnClick)中没有被处理!`;
        console.error(strMsg);
        alert(strMsg);
        break;
    }
