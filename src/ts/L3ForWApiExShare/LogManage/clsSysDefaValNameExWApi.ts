...
import {
  SysDefaValName_GetObjLstAsync,
  SysDefaValName_SortFunByKey,
  SysDefaValName_FilterFunByKey,
} from '@/ts/L3ForWApi/LogManage/clsSysDefaValNameWApi';
import { clsSysDefaValNameENEx } from '@/ts/L0Entity/LogManage/clsSysDefaValNameENEx';
 ...
 **/
export function SysDefaValNameEx_CopyToEx(
  objSysDefaValNameENS: clsSysDefaValNameEN,
): clsSysDefaValNameENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      sysDefaValNameEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function SysDefaValNameEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsSysDefaValNameENEx>> {
  ...
  if (IsNullOrEmpty(objSortInfo.SortFld) == false) {
    for (const objInFor of arrSysDefaValNameExObjLst) {
      await SysDefaValNameEx_FuncMapByFldName(objSortInfo.SortFld, objInFor);
    }
  }
  ...
  try {
    ...
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrSysDefaValName_Sel = arrSysDefaValName_Sel.sort(
        SysDefaValNameEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function SysDefaValNameEx_FuncMapByFldName(
  strFldName: string,
  objSysDefaValNameEx: clsSysDefaValNameENEx,
) {
