 ...
 /**
 * 类名:QxPotenceType_Detail(界面:QxPotenceTypeCRUD,00140041)
 * 表名:QxPotenceType(00140003)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/13 16:04:25
 * 生成者:
 工程名称:统一平台(0014)
...
import { divVarSet,refQxPotenceType_Detail } from "@/viewsShare/PotenceMan/QxPotenceTypeVueShare";
...
{
 refQxPotenceType_Detail.value.hideDialog();
}
}
...
if (QxPotenceType_Detail.strPageDispModeId == enumPageDispMode.PopupBox_01)
{
if (refQxPotenceType_Detail.value == null)
{
const strMsg = Format(
...
await refQxPotenceType_Detail.value.showDialog();
}
divVarSet.refDivDetail = refQxPotenceType_Detail.value.$refs.refDivDetail;
...
if (refQxPotenceType_Detail.value == null)
{
const strMsg = Format('当前详细信息区的DetailObj为空，请检查！(in {0}.{1}).\nrefQxPotenceType_Detail imported from: @/views/PotenceMan/QxPotenceTypeVueShare',
this.constructor.name,
strThisFuncName,
);
console.error(strMsg);
...
await refQxPotenceType_Detail.value.ShowDataFromQxPotenceTypeObj(objQxPotenceTypeEN);
...
refQxPotenceType_Detail.value.strCancelButtonText = value;
}
}