/**
 * 类名:QxRoleRightRelation_Edit(界面:QxRoleRightRelationCRUD,00140044)
 * 表名:QxRoleRightRelation(00140118)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/13 16:04:44
 * 生成者:
 工程名称:统一平台(0014)
...
import {
  ...
  QxRoleRightRelation_AddNewRecordWithReturnKeyAsync,
  ...
  QxRoleRightRelation_EditRecordExAsync,
...
import {
  QxRoleRightRelation_DeleteKeyIdCache,
  divVarSet,
...
export abstract class QxRoleRightRelation_Edit {
  ...
  public async ShowDialog_QxRoleRightRelation(strOp: string): Promise<boolean> {
    ...
    if (QxRoleRightRelation_Edit.strPageDispModeId == enumPageDispMode.PopupBox_01) {
      if (refQxRoleRightRelation_Edit.value == null) {
        const strMsg = Format(
      ...
      await refQxRoleRightRelation_Edit.value.showDialog(this);
    }
            ...
            if (returnBool == true) {
              ...
              if (this.iShowList != null)
                this.iShowList.BindGvCache(
                  clsQxRoleRightRelationEN._CurrTabName,
                  this.keyId.toString(),
                );
            }
          }
  ...
  public async AddNewRecord() {
    ...
    refQxRoleRightRelation_Edit.value.Clear();
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    let objQxRoleRightRelationEN;
    try {
      objQxRoleRightRelationEN =
        await refQxRoleRightRelation_Edit.value.GetEditDataQxRoleRightRelationObj();
    } catch (e) {
      const strMsg = Format(
        '从界面获取数据不成功,{0}.(in {1}.{2})',
        e,
    ...
    try {
      ...
      if (returnBool == true) {
        QxRoleRightRelation_ReFreshCache();
        const strInfo = `添加[角色赋权关系(QxRoleRightRelation)]记录成功!`;
        //显示信息框
  ...
  public async ShowData(lngmId: number) {
    ...
    //3、用提供的关键字初始化一个类对象；
    refQxRoleRightRelation_Edit.value.ShowDataFromQxRoleRightRelationObj(objQxRoleRightRelationEN);
  }
  ...
  public async UpdateRecord(lngmId: number): Promise<boolean> {
    ...
    try {
      ...
      await refQxRoleRightRelation_Edit.value.ShowDataFromQxRoleRightRelationObj(
        objQxRoleRightRelationEN,
      );
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    const objQxRoleRightRelationEN =
      await refQxRoleRightRelation_Edit.value.GetEditDataQxRoleRightRelationObj();
    ...
    try {
      ...
      if (returnBool == true) {
        QxRoleRightRelation_ReFreshCache();
        QxRoleRightRelation_DeleteKeyIdCache(this.keyId);
      }
      return returnBool;
    } catch (e) {
      const strMsg = Format(
  ...
  public async EditRecordExSave(): Promise<boolean> {
    ...
    try {
      ...
      const returnBool = await QxRoleRightRelation_EditRecordExAsync(objQxRoleRightRelationEN);
      if (returnBool == true) {
        QxRoleRightRelation_ReFreshCache();
      }
      return returnBool;
  ...
  public set btnCancelQxRoleRightRelation(value: string) {
    refQxRoleRightRelation_Edit.value.strCancelButtonText = value;
  }
  /**
   * 获取按钮的标题
   **/
  public get btnSubmitQxRoleRightRelation(): string {
    const strValue = refQxRoleRightRelation_Edit.value.strSubmitButtonText;
    return strValue;
  }
  /**
  ...
  public set btnSubmitQxRoleRightRelation(value: string) {
    refQxRoleRightRelation_Edit.value.strSubmitButtonText = value;
  }
}
