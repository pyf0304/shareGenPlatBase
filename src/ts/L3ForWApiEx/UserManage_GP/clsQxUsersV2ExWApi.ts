...
* Created by pyf on 2024年11月24日.
* 注意:该类必须与调用界面处于同一个包,否则调用不成功!
 **/
import { ObjectAssign } from "@/ts/PubFun/clsCommFunc4Web";
import { clsQxUsersV2ENEx } from "@/ts/L0Entity/UserManage_GP/clsQxUsersV2ENEx";
import { QxUsersV2_GetObjLstByPagerAsync,QxUsersV2_SortFunByKey } from "@/ts/L3ForWApi/UserManage_GP/clsQxUsersV2WApi";
...
export const qxUsersV2Ex_ConstructorName = "qxUsersV2Ex";
 ...
 **/
export  function QxUsersV2Ex_GetWebApiUrl(strController: string, strAction: string): string {
 ...
 **/
export  function QxUsersV2Ex_CopyToEx(objQxUsersV2ENS:clsQxUsersV2EN ): clsQxUsersV2ENEx
{
const strThisFuncName  = QxUsersV2Ex_CopyToEx.name;
...
{
const strMsg = Format("(errid:Watl000067)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxUsersV2Ex_ConstructorName, strThisFuncName);
 ...
 * @returns 对象列表
*/
export  async function QxUsersV2Ex_GetObjExLstByPagerAsync(objPagerPara: stuPagerPara):Promise<Array<clsQxUsersV2ENEx>> {
const strThisFuncName = "GetObjExLstByPagerAsync";
const arrQxUsersV2ObjLst = await QxUsersV2_GetObjLstByPagerAsync(objPagerPara);
const arrQxUsersV2ExObjLst = arrQxUsersV2ObjLst.map(QxUsersV2Ex_CopyToEx);
...
const strMsg = Format("错误:[{0}]. \n根据条件:[{1}]获取分页对象列表不成功!(In {2}.{3})", e, objPagerPara.whereCond, qxUsersV2Ex_ConstructorName, strThisFuncName);
console.error(strMsg);
