...
const strUrl = GetWebApiUrl_GP(pageDispMode_Controller, strAction);
...
{
const arrPageDispModeObjLstCache = await PageDispMode_GetObjLstCache();
const obj = arrPageDispModeObjLstCache.find(x => 
x.pageDispModeId == objPageDispMode.pageDispModeId);
if (obj != null)
{
...
strOutFldName, clsPageDispModeEN._AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (IsNullOrEmpty(clsPageDispModeEN._WhereFormat) == false)
{
strWhereCond = clsPageDispModeEN._WhereFormat;
}
if (IsNullOrEmpty(clsPageDispModeEN._CacheAddiCondition) == false)
{
strWhereCond += Format(" and {0}", clsPageDispModeEN._CacheAddiCondition);
}
if (strKey == "")
{
console.error("关键字为空!不正确");
...
switch (clsPageDispModeEN._CacheModeId)
{
...
let strUrl = GetWebApiUrl_GP(pageDispMode_Controller, strAction);
...
let arrPageDispModeSel = arrPageDispModeObjLstCache;
if (objPageDispModeCond.GetConditions().length == 0) return arrPageDispModeSel.length>0?true:false;
try {
for (const objCondition of objPageDispModeCond.GetConditions()) {
if (objCondition == null) continue; 
const strKey = objCondition.fldName;
 ...
 * @returns 对象列表记录数
*/
export  async function PageDispMode_GetRecCountByCondCache(objPageDispModeCond: ConditionCollection ) {
