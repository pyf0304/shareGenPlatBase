  ...
  try {
    ...
    if (arrXzAdminClsTypeSel.length > 0) {
      ...
      if (bolTryAsyncOnce == true) {
        const objXzAdminClsTypeConst =
          await XzAdminClsType_GetObjByIdAdminClsTypeAsync(strIdAdminClsType);
...
export async function XzAdminClsType_func(
  ...
  if (clsXzAdminClsTypeEN._AttributeName.indexOf(strOutFldName) == -1) {
    const strMsg = Format(
      '输出字段名:[{0}]不正确,不在输出字段范围之内!({1})',
      strOutFldName,
      clsXzAdminClsTypeEN._AttributeName.join(','),
    );
    console.error(strMsg);
    throw new Error(strMsg);
  }
 ...
 **/
export async function XzAdminClsType_GetFldValueAsync(
  strFldName: string,
  strWhereCond: string,
): Promise<Array<string>> {
...
export async function XzAdminClsType_GetObjLstByPagerCache(objPagerPara: stuPagerPara) {
  ...
  let arrXzAdminClsTypeSel = arrXzAdminClsTypeObjLstCache;
  const objXzAdminClsTypeCond = objPagerPara.conditionCollection;
  if (objXzAdminClsTypeCond == null) {
 ...
 **/
export async function XzAdminClsType_AddNewObjSave(
  objXzAdminClsTypeEN: clsXzAdminClsTypeEN,
 ...
 **/
export async function XzAdminClsType_UpdateObjSave(
  objXzAdminClsTypeEN: clsXzAdminClsTypeEN,
): Promise<boolean> {
 ...
 * @returns 获取修改是否成功？
 **/
export async function XzAdminClsType_EditRecordExAsync(
  objXzAdminClsTypeEN: clsXzAdminClsTypeEN,
): Promise<boolean> {
...
export async function XzAdminClsType_IsExistRecordCache(
  objXzAdminClsTypeCond: ConditionCollection,
) {
  const strThisFuncName = 'IsExistRecordCache';
  const arrXzAdminClsTypeObjLstCache = await XzAdminClsType_GetObjLstCache();
  if (arrXzAdminClsTypeObjLstCache == null) return false;
  let arrXzAdminClsTypeSel = arrXzAdminClsTypeObjLstCache;
  if (objXzAdminClsTypeCond.GetConditions().length == 0)
...
export function XzAdminClsType_CheckPropertyNew(pobjXzAdminClsTypeEN: clsXzAdminClsTypeEN) {
  ...
  if (
    IsNullOrEmpty(pobjXzAdminClsTypeEN.adminClsTypeENName) == false &&
    GetStrLen(pobjXzAdminClsTypeEN.adminClsTypeENName) > 50
  ) {
    throw new Error(
      `(errid:Watl000413)字段[行政班级类型英文名(adminClsTypeENName)]的长度不能超过50(In 行政班类型(XzAdminClsType))!值:${pobjXzAdminClsTypeEN.adminClsTypeENName}(clsXzAdminClsTypeBL:CheckPropertyNew)`,
    );
  }
  if (
  ...
  if (
    IsNullOrEmpty(pobjXzAdminClsTypeEN.adminClsTypeENName) == false &&
    undefined !== pobjXzAdminClsTypeEN.adminClsTypeENName &&
    tzDataType.isString(pobjXzAdminClsTypeEN.adminClsTypeENName) === false
  ) {
    throw new Error(
      `(errid:Watl000414)字段[行政班级类型英文名(adminClsTypeENName)]的值:[${pobjXzAdminClsTypeEN.adminClsTypeENName}], 非法,应该为字符型(In 行政班类型(XzAdminClsType))!(clsXzAdminClsTypeBL:CheckPropertyNew0)`,
    );
  }
  if (
    IsNullOrEmpty(pobjXzAdminClsTypeEN.memo) == false &&
...
export function XzAdminClsType_CheckProperty4Update(pobjXzAdminClsTypeEN: clsXzAdminClsTypeEN) {
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000416)字段[行政班级类型英文名(adminClsTypeENName)]的长度不能超过50(In 行政班类型(XzAdminClsType))!值:${pobjXzAdminClsTypeEN.adminClsTypeENName}(clsXzAdminClsTypeBL:CheckProperty4Update)`,
    );
  }
  if (
    IsNullOrEmpty(pobjXzAdminClsTypeEN.memo) == false &&
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000417)字段[行政班级类型英文名(adminClsTypeENName)]的值:[${pobjXzAdminClsTypeEN.adminClsTypeENName}], 非法,应该为字符型(In 行政班类型(XzAdminClsType))!(clsXzAdminClsTypeBL:CheckProperty4Update)`,
    );
  }
  if (
    IsNullOrEmpty(pobjXzAdminClsTypeEN.memo) == false &&
...
export function XzAdminClsType_GetCombineCondition(
  ...
  if (
    Object.prototype.hasOwnProperty.call(
      objXzAdminClsTypeCond.dicFldComparisonOp,
      clsXzAdminClsTypeEN.con_AdminClsTypeENName,
    ) == true
  ) {
    const strComparisonOpAdminClsTypeENName: string =
      objXzAdminClsTypeCond.dicFldComparisonOp[clsXzAdminClsTypeEN.con_AdminClsTypeENName];
    strWhereCond += Format(
      " And {0} {2} '{1}'",
      clsXzAdminClsTypeEN.con_AdminClsTypeENName,
      objXzAdminClsTypeCond.adminClsTypeENName,
      strComparisonOpAdminClsTypeENName,
    );
  }
  if (
    Object.prototype.hasOwnProperty.call(
