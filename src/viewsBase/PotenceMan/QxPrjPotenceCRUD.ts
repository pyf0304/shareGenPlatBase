 ...
 /**
 * 类名:QxPrjPotenceCRUD(界面:QxPrjPotenceCRUD,00140042)
 * 表名:QxPrjPotence(00140005)
 * 版本:2025.06.10.1(服务器:WIN-SRV103-116)
 * 日期:2025/06/13 16:04:29
 * 生成者:
 工程名称:统一平台(0014)
...
import { QxPrjId_Local,CombineQxPrjPotenceConditionObj4ExportExcel,CombineQxPrjPotenceConditionObj,QxPrjPotence_DeleteKeyIdCache,divVarSet,viewVarSet,dataColumn,BindTabByList,refQxPrjPotence_List } from "@/viewsShare/PotenceMan/QxPrjPotenceVueShare";
import { QxPrjPotence_GetRecCountByCondCache,QxPrjPotence_GetSubObjLstCache,QxPrjPotence_DelRecordAsync,QxPrjPotence_ReFreshCache,QxPrjPotence_GetObjByPotenceIdAsync,QxPrjPotence_FuncMapByFldName,QxPrjPotence_GetObjExLstByPagerCache,QxPrjPotence_GetObjLstByPotenceIdLstAsync,QxPrjPotence_GetMaxStrIdByPrefixAsync,QxPrjPotence_AddNewRecordAsync,QxPrjPotence_UpdateRecordAsync,QxPrjPotence_DelQxPrjPotencesAsync } from "@/ts/L3ForWApi/PotenceMan/clsQxPrjPotenceWApi";
import { QxPrjPotenceEx_CopyToEx } from "@/ts/L3ForWApiExShare/PotenceMan/clsQxPrjPotenceExWApi";
...
if (viewVarSet.sortQxPrjPotenceBy == '') viewVarSet.sortQxPrjPotenceBy = `${ clsQxPrjPotenceEN.con_PotenceId } Asc`;
 ...
 **/
public CombineData(arrQxPrjPotenceObjLst: Array<clsQxPrjPotenceEN>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
...
const strFileName = Format("工程权限({0})导出.xlsx",
 clsQxPrjPotenceEN._CurrTabName);
const strSheetName = "工程权限列表";
 ...
 **/
public CombineData4Func(arrQxPrjPotenceExObjLst: Array<clsQxPrjPotenceENEx>, arrDataColumn: Array<clsDataColumn>): ExportExcelData 
{
 ...
 **/
public async ExportExcel_QxPrjPotence4Func(): Promise<ExportExcelData> 
{
const strThisFuncName = this.ExportExcel_QxPrjPotence4Func.name;
if (viewVarSet.sortQxPrjPotenceBy == null)
{
const strMsg = Format("在显示列表时,排序字段(sortQxPrjPotenceBy)为空,请检查!(In BindGv_QxPrjPotenceCache)");
console.error(strMsg);
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
}
const objQxPrjPotenceCond = await CombineQxPrjPotenceConditionObj4ExportExcel();
 let arrQxPrjPotenceExObjLst: Array <clsQxPrjPotenceENEx> = [];
try
{

this.recCount = await QxPrjPotence_GetRecCountByCondCache(objQxPrjPotenceCond ,QxPrjId_Local.value);
if (this.recCount == 0)
{
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objQxPrjPotenceCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
return { arrObjLst: [], sheetName: '', fileName: '' };
...
const arrQxPrjPotenceObjLst = await QxPrjPotence_GetSubObjLstCache(objQxPrjPotenceCond ,QxPrjId_Local.value);
...
{
const strKey = Format("{0}_{1}", clsQxPrjPotenceEN._CurrTabName, QxPrjId_Local.value);
...
{
fldName: clsQxPrjPotenceEN.con_MenuId4Win,
sortBy: "",
sortFun: SortFun,
getDataSource: "",
colHeader: "菜单Id4Win",
...
arrQxPrjPotenceExObjLst = arrQxPrjPotenceExObjLst.sort(this.SortFunExportExcel);
 return this.CombineData(arrQxPrjPotenceExObjLst, arrDataColumn); 
...
{
const strMsg = Format("缓存分类变量:[strQxPrjId]的长度:[{0}]不正确!(.SetDdl_RoleIdInDivInFeature)", strQxPrjId.length);
console.error(strMsg);
throw (strMsg);
}
...
await QxRoles_BindDdl_RoleIdByQxPrjIdInDivCache(divVarSet.refDivFunction, "ddlRoleId" , strQxPrjId);//
}
...
if (returnInt > 0)
{
QxPrjPotence_ReFreshCache(QxPrjId_Local.value);
 ...
 **/
public async BindTab_QxPrjPotence(divContainer: HTMLDivElement, arrQxPrjPotenceExObjLst: Array<clsQxPrjPotenceENEx>) 
{
...
if (refQxPrjPotence_List.value != null){
dataColumn.value = arrDataColumn;
await BindTabByList(arrQxPrjPotenceExObjLst, this.dispAllErrMsg_q); 
}
else {
const divDataLst = GetDivObjInDivObj(divContainer, 'divDataLst');
if (divDataLst == null)
{
alert("在BindTab_QxPrjPotence函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrQxPrjPotenceExObjLst, arrDataColumn,  clsQxPrjPotenceEN.con_PotenceId, this); 
}
 ...
 **/
public async BindTab_QxPrjPotence4Func(divContainer: HTMLDivElement, arrQxPrjPotenceExObjLst: Array<clsQxPrjPotenceENEx>) 
{
...
if (divDataLst == null)
{
alert("在BindTab_QxPrjPotence4Func函数中，divDataLst不存在!");
return;
}
await BindTab(divDataLst, arrQxPrjPotenceExObjLst, arrDataColumn,  clsQxPrjPotenceEN.con_PotenceId, this); 
}
 ...
 **/
public async ExtendFldFuncMap(arrQxPrjPotenceExObjLst: Array<clsQxPrjPotenceENEx>, arrDataColumn: Array<clsDataColumn>) {
...
await QxPrjPotence_FuncMapByFldName(objDataColumn.fldName, objInFor);
}
}
}
...
await this.BindGv_QxPrjPotence4Func(this.listPara.listDiv);
}
...
const objQxPrjPotenceCond = await CombineQxPrjPotenceConditionObj();
...
lblMsg.innerHTML = Format("根据条件:[{0}]获取的对象列表数为0!", objQxPrjPotenceCond.whereCond);
const strMsg = Format("在绑定GvCache过程中,根据条件:[{0}]获取的对象列表数为0!", objQxPrjPotenceCond.whereCond);
console.error("Error: ", strMsg);
//console.trace();
alert(strMsg);
BindTabByList(arrQxPrjPotenceExObjLst, true);
return;
}
...
{
strSortFun = QxPrjPotenceCRUD.sortFunStatic(viewVarSet.ascOrDesc4SortFun);
}
const objPagerPara: stuPagerPara = {
pageIndex: intCurrPageIndex,
pageSize: this.pageSize,
whereCond: strWhereCond,
conditionCollection: objQxPrjPotenceCond,
orderBy: viewVarSet.sortQxPrjPotenceBy,
sortFun: strSortFun
}
arrQxPrjPotenceExObjLst = await QxPrjPotence_GetObjExLstByPagerCache(objPagerPara ,QxPrjId_Local.value);
}
catch(e)
{
...
const { sortFun, ascOrDesc4SortFun, sortBy } = GetSortBy(objAnchorElement, viewVarSet.ascOrDesc4SortFun, viewVarSet.sortQxPrjPotenceBy, strSortExpress);
// 将 sortBy 按空格分成两部分
const [sortColumnKey, sortDirection] = sortBy.split(' ');
if (Object.prototype.hasOwnProperty.call(clsQxPrjPotenceENEx, sortColumnKey))
{
// 调用 SortColumn 函数
this.SortColumn(sortColumnKey, sortDirection);
return;
}
viewVarSet.sortQxPrjPotenceBy = sortBy;
...
ObjectAssign(objQxPrjPotenceEN, objInFor);
objQxPrjPotenceEN.SetPotenceId(objInFor.potenceId);
//objQxPrjPotenceEN.SetRoleId(strRoleId);
let returnBool = false;
try
{
    objQxPrjPotenceEN.sfUpdFldSetStr = objQxPrjPotenceEN.updFldString;//设置哪些字段被修改(脏字段)
returnBool = await QxPrjPotence_UpdateRecordAsync(objQxPrjPotenceEN);
}
catch (e)
{
const strMsg = `设置记录不成功,${e}.(in ${this.constructor.name}.${strThisFuncName}`;
...
if (returnBool == true)
{
QxPrjPotence_DeleteKeyIdCache(QxPrjId_Local.value, objInFor.potenceId);
intCount++;
}
else
{
