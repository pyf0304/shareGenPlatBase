...
export abstract class LoginLog_Edit {
  ...
  public async ShowDialog_LoginLog(strOp: string): Promise<boolean> {
    ...
    if (strOp === 'Add' || strOp === 'AddWithMaxId') {
      ...
      this.btnCancelLoginLog = '取消添加';
    } else if (strOp === 'Update') {
      this.btnSubmitLoginLog = '确认修改';
      this.btnCancelLoginLog = '取消修改';
    }
    return true;
  }
   ...
   **/
  public async btnUpdateRecordInTab_Click(lngLoginLogId: number) {
   ...
   **/
  public async btnUpdateRecord_Click(lngLoginLogId: number) {
  ...
  public async btnSubmit_Click() {
    ...
    const strCommandText: string = this.btnSubmitLoginLog;
    try {
  ...
  public async AddNewRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Add(objLoginLogEN);
  ...
  public async ShowData(lngLoginLogId: number) {
    ...
    try {
      ...
      objLoginLogEN = objLoginLogENConst;
    } catch (e) {
      const strMsg = Format(
  ...
  public async UpdateRecordSave(): Promise<boolean> {
    ...
    try {
      //检查唯一性条件
      const bolIsExistCond = await this.CheckUniCond4Update(objLoginLogEN);
      if (bolIsExistCond == false) {
        return false;
      }
