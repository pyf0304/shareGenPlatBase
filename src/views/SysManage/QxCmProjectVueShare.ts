...
if ( cmPrjId_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxCmProjectEN.con_CmPrjId, cmPrjId_q.value);
}
if ( cmPrjName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxCmProjectEN.con_CmPrjName, cmPrjName_q.value);
}
if ( useStateId_q.value != "" && useStateId_q.value != "0")
{
strWhereCond += Format(" And QxCmProject.{0} = '{1}'", clsQxCmProjectEN.con_UseStateId, useStateId_q.value);
}

}
catch(objException)
{
const strMsg:string = Format("在组合查询条件(CombineQxCmProjectCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
...
if ( useStateId_q.value != "" && useStateId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsQxCmProjectEN.con_UseStateId, useStateId_q.value);
...
{
const strMsg:string = Format("在组合查询条件对象(CombineQxCmProjectConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
