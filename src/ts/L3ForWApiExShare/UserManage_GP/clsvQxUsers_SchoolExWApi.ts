...
import {
  vQxUsers_School_GetObjLstByPagerAsync,
  vQxUsers_School_SortFunByKey,
} from '@/ts/L3ForWApi/UserManage_GP/clsvQxUsers_SchoolWApi';
 ...
 **/
export function vQxUsers_SchoolEx_CopyToEx(
  objvQxUsers_SchoolENS: clsvQxUsers_SchoolEN,
): clsvQxUsers_SchoolENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      vQxUsers_SchoolEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function vQxUsers_SchoolEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsvQxUsers_SchoolENEx>> {
  ...
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrvQxUsers_SchoolSel = arrvQxUsers_SchoolSel.sort(
        vQxUsers_SchoolEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function vQxUsers_SchoolEx_FuncMapByFldName(
  strFldName: string,
  objvQxUsers_SchoolEx: clsvQxUsers_SchoolENEx,
) {
  ...
  let strMsg = '';
  //如果是本表中字段,不需要映射
  const arrFldName = clsvQxUsers_SchoolEN._AttributeName;
