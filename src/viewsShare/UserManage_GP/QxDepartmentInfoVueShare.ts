...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxDepartmentInfoEN.con_DepartmentId, departmentId_q.value);
}
if ( departmentName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxDepartmentInfoEN.con_DepartmentName, departmentName_q.value);
}
if ( departmentAbbrName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxDepartmentInfoEN.con_DepartmentAbbrName, departmentAbbrName_q.value);
}
if ( departmentTypeId_q.value != "" && departmentTypeId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsQxDepartmentInfoEN.con_DepartmentTypeId, departmentTypeId_q.value);
}
if ( upDepartmentId_q.value != "" && upDepartmentId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsQxDepartmentInfoEN.con_UpDepartmentId, upDepartmentId_q.value);
}
...
{
const strMsg:string = Format("在组合查询条件(CombineQxDepartmentInfoCondition)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
return strWhereCond;
};
...
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxDepartmentInfoEN.con_DepartmentId, departmentId_q.value);
objQxDepartmentInfoCond.SetCondFldValue(clsQxDepartmentInfoEN.con_DepartmentId, departmentId_q.value, "like");
}
if ( departmentName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxDepartmentInfoEN.con_DepartmentName, departmentName_q.value);
objQxDepartmentInfoCond.SetCondFldValue(clsQxDepartmentInfoEN.con_DepartmentName, departmentName_q.value, "like");
}
if ( departmentAbbrName_q.value != "")
{
strWhereCond += Format(" And {0} like '%{1}%'", clsQxDepartmentInfoEN.con_DepartmentAbbrName, departmentAbbrName_q.value);
objQxDepartmentInfoCond.SetCondFldValue(clsQxDepartmentInfoEN.con_DepartmentAbbrName, departmentAbbrName_q.value, "like");
}
if ( departmentTypeId_q.value != "" && departmentTypeId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsQxDepartmentInfoEN.con_DepartmentTypeId, departmentTypeId_q.value);
objQxDepartmentInfoCond.SetCondFldValue(clsQxDepartmentInfoEN.con_DepartmentTypeId, departmentTypeId_q.value, "=");
}
if ( upDepartmentId_q.value != "" && upDepartmentId_q.value != "0")
{
strWhereCond += Format(" And {0} = '{1}'", clsQxDepartmentInfoEN.con_UpDepartmentId, upDepartmentId_q.value);
objQxDepartmentInfoCond.SetCondFldValue(clsQxDepartmentInfoEN.con_UpDepartmentId, upDepartmentId_q.value, "=");
}
if (inUse_q.value == 'true')
{
strWhereCond += Format(" And {0} = '1'", clsQxDepartmentInfoEN.con_InUse);
...
{
const strMsg:string = Format("在组合查询条件对象(CombineQxDepartmentInfoConditionObj)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
 ...
 * @returns 条件串(strWhereCond)
 **/
export const CombineQxDepartmentInfoConditionObj4ExportExcel = async ():Promise<ConditionCollection> => 
{
//使条件串的初值为"1 = 1",以便在该串的后面用"and "添加其他条件,
//例如 1 = 1 && UserName = '张三'
const objQxDepartmentInfoCond = new ConditionCollection();
...
{
const strMsg:string = Format("在组合导出Excel条件对象(CombineQxDepartmentInfoConditionObj4ExportExcel)时出错!请联系管理员!{0}", objException);
throw strMsg;
}
objQxDepartmentInfoCond.whereCond = strWhereCond;
return objQxDepartmentInfoCond;
...
if (refQxDepartmentInfo_List.value != null) refQxDepartmentInfo_List.value.selectAllChecked = false;
};
...
if (
IsNullOrEmpty(strDepartmentId) == false )
{
// 使用 delete 删除特定的键
