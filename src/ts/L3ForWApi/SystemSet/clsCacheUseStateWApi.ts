...
export function CacheUseState_CheckPropertyNew(pobjCacheUseStateEN: clsCacheUseStateEN) {
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000413)字段[备注(memo)]的长度不能超过1000(In 缓存使用状态(CacheUseState))!值:${pobjCacheUseStateEN.memo}(clsCacheUseStateBL:CheckPropertyNew)`,
    );
  }
  //检查字段的数据类型是否正确
  if (
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000414)字段[备注(memo)]的值:[${pobjCacheUseStateEN.memo}], 非法,应该为字符型(In 缓存使用状态(CacheUseState))!(clsCacheUseStateBL:CheckPropertyNew0)`,
    );
  }
...
export function CacheUseState_CheckProperty4Update(pobjCacheUseStateEN: clsCacheUseStateEN) {
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000416)字段[备注(memo)]的长度不能超过1000(In 缓存使用状态(CacheUseState))!值:${pobjCacheUseStateEN.memo}(clsCacheUseStateBL:CheckProperty4Update)`,
    );
  }
  //检查字段的数据类型是否正确
  if (
  ...
  if (
    ...
    throw new Error(
      `(errid:Watl000417)字段[备注(memo)]的值:[${pobjCacheUseStateEN.memo}], 非法,应该为字符型(In 缓存使用状态(CacheUseState))!(clsCacheUseStateBL:CheckProperty4Update)`,
    );
  }
  //检查主键是否为Null或者空!
  if (
