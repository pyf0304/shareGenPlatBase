...
import {
  QxFuncModule_Agc_GetObjLstByPagerAsync,
  QxFuncModule_Agc_SortFunByKey,
} from '@/ts/L3ForWApi/PrjManage_GP/clsQxFuncModule_AgcWApi';
 ...
 **/
export function QxFuncModule_AgcEx_CopyToEx(
  objQxFuncModule_AgcENS: clsQxFuncModule_AgcEN,
): clsQxFuncModule_AgcENEx {
  ...
  try {
    ...
    const strMsg = Format(
      ...
      e,
      qxFuncModule_AgcEx_ConstructorName,
      strThisFuncName,
    );
 ...
 * @returns 对象列表
 */
export async function QxFuncModule_AgcEx_GetObjExLstByPagerAsync(
  objPagerPara: stuPagerPara,
): Promise<Array<clsQxFuncModule_AgcENEx>> {
  ...
  try {
    if (objPagerPara.orderBy != null && objPagerPara.orderBy.length > 0) {
      ...
      arrQxFuncModule_AgcSel = arrQxFuncModule_AgcSel.sort(
        QxFuncModule_AgcEx_SortFunByKey(strSortFld, strSortType),
      );
    } else {
 ...
 */
export function QxFuncModule_AgcEx_FuncMapByFldName(
  strFldName: string,
  objQxFuncModule_AgcEx: clsQxFuncModule_AgcENEx,
) {
  ...
  let strMsg = '';
  //如果是本表中字段,不需要映射
  const arrFldName = clsQxFuncModule_AgcEN._AttributeName;
