...
if (clsQxPrjPotenceEN._AttributeName.indexOf(strOutFldName) == -1)
{
const strMsg = Format("输出字段名:[{0}]不正确,不在输出字段范围之内!({1})",
strOutFldName, clsQxPrjPotenceEN._AttributeName.join(','));
console.error(strMsg);
throw new Error(strMsg);
}
...
if (IsNullOrEmpty(clsQxPrjPotenceEN._WhereFormat) == false)
{
strWhereCond = Format(clsQxPrjPotenceEN._WhereFormat, strQxPrjId);
}
else
{
...
if (IsNullOrEmpty(clsQxPrjPotenceEN._CacheAddiCondition) == false)
{
strWhereCond += Format(" and {0}", clsQxPrjPotenceEN._CacheAddiCondition);
}
if (strKey == "")
{
console.error("关键字为空!不正确");
...
switch (clsQxPrjPotenceEN._CacheModeId)
{
 ...
 * @returns 对象列表
*/
export  async function QxPrjPotence_GetObjExLstByPagerCache(objPagerPara: stuPagerPara , strQxPrjId:string):Promise<Array<clsQxPrjPotenceENEx>> {
const strThisFuncName = "GetObjLstByPagerCache";
...
const intEnd = intStart + objPagerPara.pageSize;
if (objPagerPara.orderBy != null && objPagerPara.orderBy.length>0) {
arrQxPrjPotenceSel = arrQxPrjPotenceSel.sort(QxPrjPotence_SortFunByExKey(objSortInfo.SortFld, objSortInfo.SortType));
}
else {
//如果排序字段名[OrderBy]为空,就调用排序函数
arrQxPrjPotenceSel = arrQxPrjPotenceSel.sort(objPagerPara.sortFun);
...
{
const strMsg = Format("(errid:Watl001294)Copy表对象数据出错,{0}.(in {1}.{2})", e, qxPrjPotence_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
 return objQxPrjPotenceENT;
}
...
*/
export  function QxPrjPotence_FuncMapByFldName(strFldName: string, objQxPrjPotenceEx: clsQxPrjPotenceENEx)
{
...
if (IsNullOrEmpty(objQxPrjPotence.funcModuleName) == true){
 ...
 const QxFuncModuleFuncModuleName = await QxFuncModule_func(clsQxFuncModuleEN.con_FuncModuleId, clsQxFuncModuleEN.con_FuncModuleName, QxFuncModuleFuncModuleId );
 objQxPrjPotence.funcModuleName = QxFuncModuleFuncModuleName;
}
}
catch (e)
{
const strMsg = Format("(errid:Watl001372)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPrjPotence_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
if (IsNullOrEmpty(objQxPrjPotence.potenceTypeName) == true){
 ...
 const QxPotenceTypePotenceTypeName = await QxPotenceType_func(clsQxPotenceTypeEN.con_PotenceTypeId, clsQxPotenceTypeEN.con_PotenceTypeName, QxPotenceTypePotenceTypeId );
 objQxPrjPotence.potenceTypeName = QxPotenceTypePotenceTypeName;
}
}
catch (e)
{
const strMsg = Format("(errid:Watl001495)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPrjPotence_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
{
const strMsg = Format("(errid:Watl001326)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPrjPotence_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
...
{
const strMsg = Format("(errid:Watl001494)函数映射表对象数据出错,{0}.(in {1}.{2})", e, qxPrjPotence_ConstructorName, strThisFuncName);
console.error(strMsg);
alert(strMsg);
}
}
